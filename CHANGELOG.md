...
- **keiro-dsl**: make mapped regeneration semantically local. Aggregate harnesses now follow only
  command-field, private-event-field, and register reachability, while one service structural
  module owns declaration-wide laws. Scaffold and diff reports distinguish checked semantic
  consumers from generated bytes; source movement is isolated to `BehaviorSourceMap`. Queue,
  public-contract, read-model-query, and aggregate-projection roots remain explicit future
  language work rather than inferred consumers.
