...
- `keiro`/`keiro-migrations`: a versioned promotion no longer inherits its
  cutover lock budget as a `statement_timeout` for the remainder of the
  transaction. Everything after the guarded lock — identity verification, the
  table renames, the promotion metadata writes, and external read
  reconciliation — ran under a timeout sized for acquiring a lock, so a
  promotion that had already taken its `ACCESS EXCLUSIVE` locks could be
  cancelled partway through with an opaque `UnexpectedServerError "57014"`
...
- **Rollout requirement:** stop/drain old timer writers, apply migration 0032,
...
- **keiro-dsl**: package and generated product records adopt concise repeated labels
  ...
  patterns; every recorded pre-v2 scaffold ledger, including legacy ledgers without an
  edition row, requires explicit, backup-backed adoption of the `idiomatic-v2`
  generated-Haskell edition. Legacy sidecar/name migration composes with edition
  adoption in one run requiring both flags. `.keiro` syntax, serialized identities, and
...
- **keiro-dsl**: regenerate each mapped service once, reconcile the generated Cabal fragment with
  ...
  `semantic-impact` baseline. This changes generated layout and evidence only; wire, fold,
...
- **keiro-dsl**: make mapped regeneration semantically local. Aggregate harnesses now follow only
  ...
  module owns declaration-wide laws. Scaffold and diff reports distinguish
  checked semantic consumers from generated bytes; source movement is isolated
  to `BehaviorSourceMap`. Public contracts and heterogeneous category/all
  projection sources remain explicit unsupported boundaries rather than
  inferred consumers.
- **keiro-dsl**: qualify candidate language 5's complete mapped consumer surface
  with one compiled event/queue/query/projection service, exact single/workspace
  locality under unrelated growth, and byte-restoring mutations for every
  semantic authority. Adoption guidance separates event upcasting, queue drains
  or transitional codecs, query caller recompilation, projection review/rebuild,
  snapshot hydration, and application-owned SQL migration.
