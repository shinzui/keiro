...
escalationEventStream = EventStream
  ...
  B.buildTransducer EscalationIdle RNil (const False) do
    B.from EscalationIdle do
      B.onCmd inCtorNoteRaised $ \d -> B.do
        B.emit wireRaiseNoted RaiseNotedTermFields
    ...
    B.from Awaiting do
      B.onCmd inCtorNoteAcknowledged $ \d -> B.do
        B.emit wireAcknowledgeNoted AcknowledgeNotedTermFields
          { incidentId = d.incidentId
          }
        B.goto Settled

escalationCodec :: Codec EscalationEvent
escalationCodec = Codec
...
escalationTimerRequest incidentId fireAt = TimerRequest
