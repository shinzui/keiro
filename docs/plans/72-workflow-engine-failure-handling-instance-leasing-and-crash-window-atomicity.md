...
- [ ] M2: rework `WorkflowResumeOptions` (drop `useAdvisoryLock`; add `maxAttempts`, `leaseTtl`, `logEvent`) and add `ResumeLogEvent`
- [ ] M2: per-instance exception/transient classification in `resumeWorkflowsOnce` (`Error StoreError :> es` constraint on the worker functions; `catchError` innermost, sync-exception catch outermost)
- [ ] M2: terminal-failure marking (append `WorkflowFailed` once attempts reach `maxAttempts`; flip a child instance's `keiro_workflow_children` row to `failed`)
- [ ] M2: pass-level catch-and-continue in `runWorkflowResumeWorkerWith` and `runWorkflowResumeWorkerPush`
- [ ] M2: extend `ResumeSummary` (`failed`, `transientErrors`, `leaseSkipped`) and `KeiroMetrics` (`workflowFailed`, `workflowResumeErrors`, `workflowLeaseSkipped`)
- [ ] M2: tests — poison + healthy isolation, max-attempts terminal failure, transient store error survives, fixed-poll and push loop drivers survive a throwing pass
