...
- [x] M4: make `signalAwakeable` commit the row transition and the journal append in one transaction; add the `Completed`-row repair to `awaitCancellable`'s arm (completed 2026-06-15)
- [x] M4: make `childCompletionHook` commit `markChildResultTx` and the parent journal append in one transaction, branching on the child row's actual status (skip on cancelled — finding M3 part 2); add the `ChildCompleted`-row repair to `awaitChild`'s arm (completed 2026-06-15)
- [x] M4: crash-window tests — completed awakeable row without journal entry heals on resume; completed child row without parent entry heals on resume (completed 2026-06-15)
- [x] M5: make `cancelChild` atomic (row flip + child `WorkflowCancelled` marker + parent sentinel in one transaction) and make `runChildWorkflow` heal a cancelled-but-unmarked child (completed 2026-06-15)
- [x] M5: tagged child-result envelope (`{"ok": …}` / `{"cancelled": true}` / `{"failed": …}`) with legacy fallback decode; replace `awaitChild`'s partial `error` with `WorkflowStepDecodeError`; add `WorkflowChildFailed` and parent propagation of a failed child (completed 2026-06-15)
- [x] M5: re-check the cancellation marker on every step/await/patch miss in the `runWorkflowWith` handler (new internal `WorkflowCancelPending` unwind) (completed 2026-06-15)
- [x] M5: tests — crash-after-flip cancel heals, retried cancel appends, honest `{"cancelled": true}` child result is delivered not thrown, decode failure is typed, failed child wakes parent with `WorkflowChildFailed`, mid-run cancel stops remaining steps (completed 2026-06-15)
- [x] M6: haddock truth pass (`signalAwakeable`, `cancelChild`, `childCompletionHook`, `Awakeable.hs` and `Child.hs` "resume worker treats it as a failure" claims, module headers) (completed 2026-06-15)
- [x] M6: run `cabal build all`, `cabal test keiro-test`, `cabal test keiro-migrations-test`, `cabal test jitsurei-test`; fix any compile fallout (e.g. `WorkflowOutcome` matches in jitsurei) (completed 2026-06-15)
- [x] M6: tick the three EP-6 checkboxes in `docs/masterplans/9-keiro-production-readiness-hardening.md` and write the Outcomes & Retrospective entry here (completed 2026-06-15)
...
- Implementation discovery (2026-06-15): the child-result envelope can be introduced without a schema migration. `keiro_workflow_children.result` continues storing the raw child result, while parent await-step journal entries wrap successful results as `{"ok": ...}`; existing raw parent entries remain readable through the legacy fallback.
...
Milestone 4 is complete as of 2026-06-15. `signalAwakeable` and `childCompletionHook` now compose their row transition with the parent/workflow journal append in one transaction, and the corresponding await arms repair rows that reached `completed` before a journal entry was appended.
...
Revision note (2026-06-15): Milestone 4 is now complete. `signalAwakeable` and `childCompletionHook` use `prepareJournalAppend` to compose row status changes with journal appends, `awaitCancellable` and `awaitChild` repair completed rows without requiring a second external signal/hook, and validation passed with `cabal test keiro-test --test-options='--match "Keiro.Workflow.Awakeable"'` (8 examples, 0 failures), `cabal test keiro-test --test-options='--match "Keiro.Workflow.Child"'` (8 examples, 0 failures), and full `cabal test keiro-test` (231 examples, 0 failures).

Revision note (2026-06-15): Milestone 5 is now complete. `cancelChild` composes the child cancellation marker and parent sentinel with the row transition, `runChildWorkflow` repairs historically cancelled-but-unmarked children, child await results are tagged envelopes with typed cancellation/failure/decode errors, failed children wake their parents, and `runWorkflowWith` stops at cancellation boundaries during a run. Validation passed with `cabal test keiro-test --test-options='--match "Keiro.Workflow.Child"'` (14 examples, 0 failures), `cabal test keiro-test --test-options='--match "Keiro.Workflow"'` (68 examples, 0 failures), and full `cabal test keiro-test` (237 examples, 0 failures).

Revision note (2026-06-15): Milestone 6 is complete and EP-6 is fully implemented. The workflow haddocks now match the shipped failure semantics, including atomic awakeable/child wake writes, cancel repair, child result envelopes, failed-child propagation, resume-worker attempt/backoff behavior, and mid-run cancellation boundaries. Final validation passed with `cabal build all`, `cabal test keiro-test` (237 examples, 0 failures), `cabal test keiro-migrations-test` (2 examples, 0 failures), and `cabal test jitsurei-test` (16 examples, 0 failures).
