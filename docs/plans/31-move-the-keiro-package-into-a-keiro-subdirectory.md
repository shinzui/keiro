...
- [x] M3 (follow-up): Closed the pre-existing gap so the DB-backed suites self-provision their schema. Wired `runAllKeiroMigrations` (LaxCheck) into the `withTestStore`/`withTwoContexts` helpers of `keiro/test/Main.hs` and `jitsurei/test/Main.hs` (added `keiro-migrations`, `codd`, `attoparsec`, `containers` test deps to both packages), and fixed one jitsurei test that needed `initializeJitsureiTables` after dispatch began running the target inline projection. All suites now pass: `keiro-test` 81/0, `jitsurei-test` 16/0, `keiro-migrations-test` 1/0. (done 2026-05-28)
...
- Decision (follow-up M3): Make the DB-backed test suites self-provision their schema by applying `runAllKeiroMigrations` (with `LaxCheck`) to each freshly started ephemeral database inside the existing `withTestStore`/`withTwoContexts` helpers, rather than relying on an externally pre-migrated/warmed database.
  Rationale: After the move the suites still could not run in a clean shell because nothing applied the kiroku/keiro schema (root-caused in Surprises & Discoveries). `keiro-migrations-test` already proved the exact pattern — `Pg.withCached` then `runAllKeiroMigrations coddSettings 5s LaxCheck` — so the suites now reuse it. `LaxCheck` is correct because ephemeral test databases keep no checked-in codd expected-schema representation, so strict verification would (noisily) flag environment-specific role/db-setting differences. Applying migrations per test preserves the existing per-test isolation (each `it` gets a pristine database) at a modest, measured wall-clock cost. This closes the gap the original plan had explicitly deferred; it is recorded here because the user asked to fix the tests after the move landed.
  Date: 2026-05-28
