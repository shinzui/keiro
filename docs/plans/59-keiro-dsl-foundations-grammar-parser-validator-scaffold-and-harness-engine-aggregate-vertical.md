...
- [ ] Write `Keiro.Dsl.Scaffold` exposing `ScaffoldModule`/`ModuleKind` and `scaffoldAggregate :: Context -> Aggregate -> [ScaffoldModule]`.
- [ ] Emit the symbol-free deterministic layer into `Generated` modules (domain ADTs, id newtypes, codec, EventStream/Projection wiring, `…Regs`, `initial…Regs`, the TH splice).
- [ ] Emit the typed hole module (`Holes.hs`) with the transducer signature, the `buildTransducer` skeleton, and per-`guard`/`write`/`emit` annotation holes.
- [ ] Add the `scaffold <file> --out <dir>` subcommand with `Generated`=overwrite / `HoleStub`=create-if-absent file-writing discipline.
- [ ] Write the firewall-invariant test (no `Generated` module text contains a keiki symbolic operator).
- [ ] Write the idempotence test (run `scaffold` twice; a hand-edited `Holes.hs` is left untouched).
- [ ] Capture the `HospitalCapacity/Reservation` reference modules under `keiro-dsl/test/fixtures/`; show the scaffolded `Generated` modules compile.
- [ ] Scaffold the register-free smoke target (`OrderStream`); show it compiles.
