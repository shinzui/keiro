...
- [x] (2026-08-09 21:52Z) Milestone 4: audited existing mapped-closure consumers, amended ADR
  0012, and passed the complete package, target-document, ADR, and corpus gates without changing
  generated output.
...
- The current corpus policy selects 35 scaffold invocations, not the MasterPlan's planned count of
  41. The committed checkpoint allowed all 35 to run, and the policy reported suite coverage,
  record/disk consistency, Cabal inventory consistency, and generated bytes clean. EP-6 must use
  the policy's live inventory rather than pinning the stale estimate.
...
EP-1 delivered one public `Keiro.Dsl.SemanticImpact` authority over the resolved `TypeGraph`.
Command-field, private-event-field, and register roots now produce deterministic aggregate
closures, inverse declaration-consumer sets, and a complete service inventory that retains unused
declarations. The root fold is compile-time exhaustive, and its `Mapped`-prefixed public
constructors avoid the existing grammar-level `RegisterRoot` name.

The technical request now separates declaration-owned service conformance from aggregate-use
evidence and states the current mapped-root boundary without inventing queue, public-contract,
read-model, or projection consumers. ADR 0012 records that authority durably. The source audit
assigns generator migration to EP-2 and report/history migration to EP-5 while preserving
path-sensitive `MappedDiff`, coverage, and binding explanations.

Validation passed with four focused semantic-impact examples, the complete 619-example
`keiro-dsl-test` suite, `cabal build keiro-dsl`, strict ADR/profile/log enforcement, the current
`just adr-validate` recipe, target-scoped strict IR profile validation, and all 35 selected
conformance-corpus invocations. The corpus left the worktree byte-clean. Whole-project IR
validation remains red on twelve unrelated historical `status: implemented` values; it reports no
IR-21 defect. No generator behavior or generated Haskell bytes changed in this plan.
...
report no generated-file drift because this plan changes no emitter. At the completed EP-1
checkpoint, the whole improvement-request validator reports twelve unrelated historical
`status: implemented` values; it must not report IR-21, whose document passes the declared profile
when validated in isolation.
...
The complete `keiro-dsl-test`, package build, ADR validation, target IR profile validation, corpus
policy, and diff hygiene pass. No file under `keiro-dsl/test/conformance-*/Generated` changes. The
whole improvement-request bundle's twelve pre-existing status diagnostics remain separately
visible and do not identify IR-21.
