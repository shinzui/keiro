...
- [x] (2026-08-05 08:48 PDT) Milestone 3: add `Keiro.Dsl.CheckReport` writing schema
  `keiro-dsl/check-report/1` from both check paths via `--report-out`, with shape tests and the
  public-CLI golden `keiro-dsl/test/fixtures/check-report/legacy-min-language.golden.json`.
  The `check report` focused group passes 3 examples covering error, denied-warning, allowed-warning,
  canonical workspace, workspace related-location, and pre-parse failure behavior.
- [x] (2026-08-05 08:55 PDT) Milestone 4: ratify the warning-policy review in the Decision Log,
  publish the check reference, validation model, copy-pasteable CI recipe, ADR 0004 amendment,
  ADR log entry, and both changelogs. `nix develop -c cabal test keiro-dsl` passes 575 examples
  with zero failures, `nix develop -c cabal build all` succeeds, strict ADR validation reports
  21 valid concepts, and diff hygiene is clean.
...
- Observation: invoking `nix develop` from `keiro-dsl/` rather than the repository root triggers
  the development shell's PostgreSQL bootstrap and created a disposable 39 MB `keiro-dsl/db`
  directory. It was removed immediately; all plan commands remain rooted at the repository root as
  Concrete Steps requires. The package-relative golden was still generated by the public CLI from
  that invocation before cleanup.

- Observation: EP-194 and EP-197 need no report-schema revision when they append diagnostic
  codes. `keiro-dsl/check-report/1` represents codes as open strings, while workspace reports
  retain per-member provenance and related locations; later validations can flow through the
  existing source/workspace builders without adding a parallel machine contract.
  Evidence: the report golden and workspace tests cover code/severity/location projection, and
  the module contract requires append-only keys with unknown-key tolerance.
...
- Decision: Ratify the three warning-policy classes unchanged after implementation: operational-
  history and deliberate-policy findings remain warnings, while `WireSchemaVersionMismatch` and
  `RmProjectionWithoutNode` remain future strict-profile candidates. Per-invocation denial is the
  shipped enforcement mechanism for all three classes under released language 4.
  Rationale: implementation and test evidence found no additional fact that moves a warning to an
  earlier sound boundary, and retroactive severity tightening would change the accepted language-4
  surface. The user guide and ADR 0004 now publish the same policy.
  Date: 2026-08-05
...
- Decision: A workspace report uses top-level language `sourceForm: "workspace-composed"` and
  `declaredLanguageVersion: null`; each member retains its own declared-versus-legacy provenance in
  the append-only `members` array.
  Rationale: workspace composition proves one effective contract, but it permits source-form
  differences that select the same version (for example legacy-unversioned and declared version 1).
  Claiming one top-level declaration would invent provenance; the effective version/runtime/support
  remain authoritative at the top while member rows state how each file selected it.
  Date: 2026-08-05


## Outcomes & Retrospective

EP-193 is complete. Compatibility-only checks and scaffolds now print exactly one line:
...
Diff applies it only to the working-tree side. `--min-language`, `--deny-warnings`,
repeatable/comma-separated `--deny`, and `--report-out` give CI a binary gate plus structured
evidence without changing warning severity.

The focused `language support`/`source language version`, `warning enforcement`, and `check
report` groups pass 4, 21, 3, and 3 examples respectively. The complete suite passes 575 examples
with zero failures. The committed public-CLI golden is
`keiro-dsl/test/fixtures/check-report/legacy-min-language.golden.json`. The pre-existing exact
stderr assertions changed only for the declared-v1 check and two contextual-identifier checks;
the scaffold assertions were already contains-based.

ADR 0004 now records the effective-contract/floor gate, the appended
`LanguageVersionBelowMinimum` code, unchanged-severity per-invocation denial, and the
append-only/ignore-unknown `keiro-dsl/check-report/1` contract (including workspace members and
the parse/compose boundary). EP-194 and EP-197 can append codes and emit their diagnostics through
this schema without further preparation. A CI invocation combining `--min-language 4`,
`--deny-warnings`, and `--report-out` can therefore prove from exit status and the report that the
checked graph used language 4 and tolerated zero warnings. The all-package build, strict 21-concept
ADR validation, and diff hygiene also pass; no generated bytes or conformance fixtures changed.
