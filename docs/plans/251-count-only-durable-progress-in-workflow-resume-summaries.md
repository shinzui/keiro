...
- [x] (2026-08-13 15:20Z) M4: marked EP-2 Complete in the MasterPlan, completed the ADR distillation and retrospective, and passed both `just haskell-test` and the repository-wide `just verify` gate.
...
- The red test exhausted all five allowed passes instead of stopping after one: the
  due sleep was rediscovered and reported `advanced = 1` on every pass even though
  its journal, counter, instance status, and wake hint were unchanged. This gave a
  direct behavioral measurement of the defect rather than only a summary-field
  assertion.
- Adding the public `WorkflowRunOptions.onJournalAppend` field exercised generated
  workflow conformance packages during the full gate. No generated bytes drifted,
  and the complete 39-invocation DSL scaffold corpus remained consistent.
...
- Decision: `ContinuedAsNew` is counted through the witness like everything else
  ...
  Date: 2026-08-12
- Decision: The completion distillation amends ADR 0023 and ADR 0025 rather than
  creating a third workflow-resume ADR.
  Rationale: ADR 0023 already owns exact discovery and the durable wake ledger, while
  ADR 0025 already owns partial-progress summaries and drain-loop behavior. Their
  amendments now record the durable `sleepDue` classification and witness-based
  `advanced` contract; the remaining observations are task-local evidence.
  Date: 2026-08-13
...
Completed on 2026-08-13. `ResumeSummary.advanced` now counts only observable durable
movement: a fresh journal append, a crash-ceiling terminal failure, or an external
wake observed mid-pass. A due unresolved sleep with no timer worker reports
`advanced = 0` and `sleepDue = 1`, so the documented bounded drain stops after one
pass while leaving the candidate discoverable for the timer worker to repair.

The operator surface renders the blocked cause as the human `sleep_due` column and
JSON key, and the runtime/operator guides name the timer worker as the remedy. The
fix required no schema migration. ADR 0023 and ADR 0025 carry the durable discovery,
classification, and drain semantics; no additional ADR was necessary.

The focused regression moved from five exhausted passes to one terminating pass,
all 545 `keiro-test` examples and 42 `keiro-ops-test` examples passed, strict ADR
validation reported 33 concepts, and both `just haskell-test` and `just verify`
completed successfully. No planned gap remains in EP-2.
...
The completion gates both exited successfully:

```text
just haskell-test
  keiro-test: 545 examples, 0 failures
  keiro-ops-test: 42 examples, 0 failures
  generated jitsurei diagrams: up to date

just verify
  process-compose dry run, migrations/demo, cabal build, package tests,
  39-invocation DSL conformance corpus, and migration tests: passed
