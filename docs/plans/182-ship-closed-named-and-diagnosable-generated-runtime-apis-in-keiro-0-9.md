...
- [x] 2026-08-02 13:15 PDT: Milestone 4 stamped every planned Generated
  module with the running package version, effective language version, and
  stable node origin; unified strict recognition of the historical and stamped
...
- Discovery (Milestone 4, 2026-08-02): `ScaffoldModule.origin` includes source
  line numbers for collision diagnostics, but source locations are not stable
  generated identity.  Stamping that text verbatim made an unchanged aggregate
  moved between workspace members rewrite its files.  Banner provenance now
  retains the node kind/name and strips only a terminal `(line N)` suffix;
  collision reports keep the complete origin metadata.
...
- Decision: Stamp provenance with the package version compiled into the
  ...
  Date: 2026-08-02
- Decision: Apply provenance stamps at the successful planning boundary (and
  defensively at single-file execution), not inside each raw emitter.  A stamp
  uses stable node identity and excludes diagnostic source-line/member-path
  metadata.
  Rationale: the checked language contract is authoritative at planning, one
  shared function then covers every Generated module kind, raw emitters remain
  composable, and workspace ownership moves remain content-neutral.  Planning
...
Milestone 4 completed on 2026-08-02.  Planned files now carry the frozen form
`-- @generated by keiro-dsl <package-version> (language keiro-dsl
<effective-version>) from <stable-node-origin>; do not edit.`.  The current
tree truthfully emits package `0.8.0.0`; the later coordinated Cabal version
bump changes that value without another generator edit.  Overwrite preflight,
stale evidence, and workspace adoption recognize only the exact historical
