...
- Observation: The checked Milestone 1 inventories found fewer package fields but more generated
  declarations than the preliminary lexical scan suggested.
  Evidence: `python3 scripts/generate-record-migration-manifests.py --check` accounts for 285
  package record-owning declarations with 1,401 fields, including 1,363 strict fields and 1,270
  fields exported through an exposed module's `Type (..)`. The generated edition inventory covers
  487 overwriteable modules, 336 record declarations, 1,463 fields, 8,768 lexical field-label
  occurrences, 154 local default pragmas, and 122 hand-owned conformance modules.
  Impact: These generated documents, rather than the reconnaissance counts, are the migration and
  omission-detection authorities for the remaining milestones.

- Observation: Twelve package and forty-seven generated record declarations place a single field
  on the declaration line rather than the following Fourmolu record block.
  Evidence: Extending the inventory parser to recognize inline braces added `Loc`, `BehaviorKey`,
  the nominal/type-graph unwrappers, and 47 generated newtypes; the deterministic check then moved
  from 273/289 to 285/336 record-owning declarations.
  Impact: Inline newtypes need the same explicit-accessor and edition treatment as multiline
  records. The inventory checker now covers both repository formatting shapes.
...
- Decision: Keep the generated consumer-occurrence inventory deliberately lexical and
  over-approximating, while deriving declaration rows from the repository's one-field-per-line
  Fourmolu house style.
  Rationale: A lexical occurrence table includes harmless bindings and patterns, but it cannot
  silently miss a direct selector spelling. Compiler diagnostics and human review can narrow those
  rows during the edition rewrite, while `--check` makes declaration, pragma, and consumer drift a
...
2026-08-23: Completed Milestone 1 with deterministic package and generated-Haskell migration
