...
- [x] 2026-09-17: M4. `Map[Id] V` parses and pretty-prints under a Language 6 syntax feature,
      resolves as a keyed map whose key is a nominal ID leaf, and generates a shape keyed by
      the ID's Haskell type with admission on every key.
- [x] 2026-09-17: M4. Consumer-bound key types carry an ordering obligation that conformance
      checks against canonical TypeID text order; coverage, diff, and the corpus cover keyed maps.
      Evidence: commits `d5ff8e3f` and `b7831b85`; the 755-example unit suite and focused
      structural-nominals suite pass, the consumer-order mutation turns its named assertion
      red before exact restoration, and two consecutive 47-invocation corpus replays are clean.
- [x] 2026-09-17: M5. `AggregateTypeUnsupportedAtUse` for `Optional <nominal>` names the
      one-field structural record pattern; documentation, changelog, and the ADR are updated;
      the plan 287 corpus and goldens are regenerated once. The focused contract suite, strict
      ADR validation, and improvement-request profile validation pass.
...
- 2026-09-17 implementation: Router selection needs both forms of a nominal projection:
  ...
  nominal binding.
- 2026-09-17 implementation: A single consumer-key fixture cannot detect an `Ord` instance
  that disagrees with canonical TypeID text order. Adding a second distinct `ClaimId` fixture
  made the pairwise ordering law meaningful; reversing the consumer `Ord` then turns the
  named conformance assertion red while the baseline remains green.
- 2026-09-17 implementation: GHC 9.12.4's optimizer panicked while simplifying generated
  keyed-map codec bindings when nominal key helpers were inlined at `-O1`. Keeping the small
  parse/render key helpers behind `NOINLINE` preserves the generated authority and compiles
  reliably; the compiled structural-nominals suite exercises the resulting boundary.
- 2026-09-17 implementation: Emitting key helpers for every nominal ID changed unrelated
  Language 6 corpora. The clean 47-invocation replay exposed the locality violation, so the
  emitter now derives the exact keyed-ID set from resolved structural expressions and emits
  helpers only for IDs used at keyed-map key positions.
...
- Decision: A keyed map's shape is a Haskell `Map` keyed by the ID's domain type. Generated
  ...
  Date: 2026-09-17
- Decision: Emit nominal key helpers only for ID declarations reached at an `RKeyedMap` key
  position, and retain `NOINLINE` on those helpers as a tested GHC 9.12.4 optimizer boundary.
  Rationale: Key codecs are a use-site capability, not a property every nominal module needs;
  exact reachability preserves unrelated generated bytes, while the explicit optimization
  boundary avoids a compiler panic without changing wire behavior or consumer obligations.
  Date: 2026-09-17
- Decision: Keep the direct-optional diagnostic concise and put the complete one-field
  structural declaration pattern in the typed-toolchain guide.
  Rationale: The diagnostic must remain useful at a source location without becoming a long
  embedded tutorial, while the guide can explain declaration, field policy, and usage together.
  Date: 2026-09-17
...
context, codec, conformance, and fingerprint authorities before implementation. All five
milestones are complete. Generated and consumer-bound enums share the nominal structural leaf
authority; declared contract IDs reuse their declaration's type and admission; nested nominal
paths participate in equality, exact projections, and declarative router selection; and
`Map[Id] V` preserves the nominal key domain through generated Haskell, codecs, coverage,
compatibility diffing, and conformance. Direct optional IDs remain deliberately unsupported,
with an exact wrapper-pattern diagnostic and complete typed-toolchain guidance.

The final evidence is a 755-example unit suite with zero failures, focused compiled contract
and structural-nominals suites, a consumer-order mutation that turns its named assertion red,
two consecutive byte-stable 47-invocation corpus replays, and clean strict ADR and
improvement-request validation. Key-helper emission is local to actual keyed-map uses, so
unrelated Language 6 corpora retain their prior bytes. The staged-release recommendation still
applies: these features are independently evidenced, and public contract containers plus direct
aggregate containers remain outside the delivered surface.
