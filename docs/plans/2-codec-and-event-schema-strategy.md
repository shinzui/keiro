...
- [ ] M0.1 — Read `hindsight` prior art (`/Users/shinzui/Keikaku/hub/haskell/hindsight/hindsight-core/src/Hindsight/Events.hs`, `.../Events/Internal/Versioning.hs`, `.../Events/Internal/TypeLevel.hs`, `.../event-test-lib/Test/Hindsight/Generate.hs`, plus `docs/OVERVIEW.md` and the README) and write a one-page evaluation note covering: which mechanisms apply to keiro (type-level event names, `MaxVersion`/`Versions` families, consecutive `Upcast` composition, `parseMap`, auto-generated roundtrip/golden tests), which do not (anything that conflicts with keiki's `SymTransducer co` output type or with kiroku's `EventData.eventType :: Text` registry), and the cost/benefit of compile-time vs value-level versioning given keiro's constraints. Land the note as a temporary scratch file under `spikes/codec/notes/hindsight-evaluation.md` (drop or fold into the design doc at M2).
- [ ] M0.2 — Decide a tentative verdict (adopt / selectively borrow / reject) and record it in this plan's Decision Log. M1's spike API and M2's design doc are gated on this verdict — if the answer is "adopt", revise the `Codec e` shape in the Plan of Work and Interfaces sections before starting M1.
