...
- [x] (2026-07-31T17:36:04Z) Milestone 3: declared version 1 in all 211 ordinary pre-existing
  fixtures and every generated skeleton; added the six-fixture compatibility/negative allowlist
  plus a declared-v1 fixture; updated exact line-sensitive diagnostics, the workspace diff golden,
  authoring/user/corpus documentation, the changelog, and accepted ADR 16. The focused suite passed
  15 examples, the full suite passed 394 examples, `cabal build all` succeeded, native
  `nix flake check` passed its pre-commit and treefmt checks, and strict ADR validation accepted all
  16 concepts.
...
- 2026-07-31: Adding a preamble shifts the parser's in-memory source locations, so
  ...
  unchanged.
- 2026-07-31: The corpus guide's claimed complete 121-fixture inventory had fallen behind the
  actual 211-file pre-migration corpus. It now identifies the 218-file machine-checked total and
  labels its long-form table as curated, while a dedicated table documents every source-language
  contract fixture.
- 2026-07-31: `nix flake check` passed the native `aarch64-darwin` pre-commit and treefmt checks.
  Nix reported the repository's usual warning that checks for incompatible non-native systems
  were omitted; this is not a failed native check.
...
The delivered contract matches the purpose. A `.keiro` source now selects a released parser before
body parsing, explicit v1 survives parse/check emission, future versions fail once with a stable
source-selection code, and legacy input remains truthfully unversioned with effective version 1.
Single-source and canonically ordered workspace JSON inspection expose that provenance. Workspace
composition retains it per member, rejects unequal effective versions before merge, and keeps the
merged `Spec` semantic.

Scaffold history records source provenance additively and reports declaration drift without
changing generated bytes or ownership. Source-aware diff reports a header-only rewrite as one
all-compatible, no-action finding; semantic diffs, fold surfaces and fingerprints, and replay
impact remain unchanged. All ordinary Keiro-owned fixtures and `new <kind>` starters declare v1,
while the explicit six-file compatibility/negative allowlist keeps legacy and refusal behavior
executable.

ADR 16 was accepted after its dispatch, wrapper, workspace, history, and replay-neutrality claims
were covered by the passing suite. No additional durable decision emerged beyond that ADR. The
remaining deliberate gap is automated sequential source/workspace upgrades and Mori-aware fleet
rewrites, still owned by IR-5; no dependent repository was changed.
...
and [plan 161](161-add-authoritative-typed-scalar-aggregate-expressions.md).
...
```text
keiro-dsl/test/fixtures/language-future.keiro:1:1: error [UnsupportedLanguageVersion]: declared keiro-dsl language version 2 is unsupported; supported versions: 1
```
...
member's declaration and effective version. The landed fixture emits:

```json
{
  "kind": "workspace",
  "members": [
    {"declaredLanguageVersion": 1, "effectiveLanguageVersion": 1, "path": "domain/project-artifact.keiro", "sourceForm": "declared"},
    {"declaredLanguageVersion": 1, "effectiveLanguageVersion": 1, "path": "domain/project.keiro", "sourceForm": "declared"},
    {"declaredLanguageVersion": 1, "effectiveLanguageVersion": 1, "path": "domain/shared.keiro", "sourceForm": "declared"}
  ],
  "path": "keiro-dsl/test/fixtures/workspace/service.keiro-workspace",
  "schema": "keiro-dsl/source-inspection/1",
  "service": "demo-project"
}
```

The `rg --files-without-match` output must contain only
...
Revision note: Completed Milestone 3 and the ExecPlan on 2026-07-31, migrating the Keiro-owned
corpus and starters to declared v1, preserving the explicit compatibility allowlist, accepting ADR
16, and recording the final CLI, test, build, Nix, and governed-document validation evidence.
