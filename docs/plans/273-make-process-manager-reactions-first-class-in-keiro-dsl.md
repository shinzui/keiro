...
provenance:
  ...
  revisions:
    ...
    - model: "gpt-5.6-sol"
      harness: "codex-cli"
      at: 2026-09-15T23:32:05Z
      mode: "implement"
      note: "Implement process-manager reaction grammar, checking, generation, evolution diagnostics, conformance, and closeout."
---
...
- [x] (2026-09-15 23:32Z) Reconcile M1 with the current tree: Language 6 is already the active candidate for delegated inbox syntax, so reactions extend that candidate in place and keep runtime profile 5.
- [x] (2026-09-16 01:10Z) M1: extend the existing Language 6 candidate with `ProcessReactionSyntax`, the `ProcessBody` sum, complete reaction/timer syntax, canonical rendering, frontend profiles, and a valid authoring skeleton while preserving the legacy process branch.
...
- [x] (2026-09-16) M4: complete timer terminal/absence/claimed-race/ceiling coverage, typed-silent and no-advance state-authority coverage, the four-axis mutation proof, and the `process-scaling` benchmark (67,582/72,142/90,382 generated bytes and 3.78/4.06/5.38 ms at 8/32/128 dispatches on the implementation machine).
- [x] (2026-09-16) M4 hydration proof: add default-disabled runtime entry probes and an isolated verification script. Same- and distinct-target fan-out at 8/32/128 each records one saga hydrate on first delivery, zero on replay with one witness, and target counts of 8/32/128; no-advance records zero, and the synchronized 32-way conflict recorded 101 saga hydrate entries, including retry attempts.
- [x] (2026-09-16) M5: add reaction/arm/timer/version/fingerprint diff codes, process coordination snapshots and scaffold drift, explicit legacy identity-model migration refusal, and checked diff fixtures. The focused diff group passes 105 examples; the same-path CLI reports advisory-only reordering at version 2 and exits 1 for the legacy migration.
- [x] (2026-09-16 04:36Z) M6: update user documentation, guides, the authoring skill, changelogs, OKF logs, ADR-41 with DSL-specific decisions, and the completed IR-39 record. The registered IR profile validates all 39 records with profile/log enforcement, strict ADR validation passes all 43 records, and `just verify` exits 0.
...
During M4 implementation, lazy bytestring writes to the shared stderr handle interleaved under the
32-way conflict probe. Switching only the opt-in marker writes to one strict bytestring operation
restored the promised one-JSON-object-per-line protocol. The default build still compiles those
imports and calls out. Cabal's component-wide `cpp-options` change recompiles the runtime library,
which is why the verification script uses one fresh temporary build tree for both instrumented
executables rather than toggling the ordinary build in place.

M4's work paragraph asked for a no-advance arm “with effects,” but the normative notation section
and generator contract in this same plan define a whole-arm `no-action` as `NoAdvance []`. The
implemented conformance follows that normative closed grammar: it proves zero saga hydration and
zero effects for no-advance. Unconditional effects after a typed silent advance are covered
separately by `ResponderIgnored`, including redelivery, so the runtime boundary remains tested
without inventing an unparsed no-action-with-effects syntax.

Implementation resumed on 2026-09-15 against a tree where commit `6b16d217` had already opened
Language 6 for `DelegatedInboxSyntax`, added runtime profile 5, moved the authoring baseline to 6,
and restored `candidate-primary`. Reaction syntax itself remained absent. ADR-16 permits correcting
the one active unpublished candidate in place, so allocating Language 7 would incorrectly freeze a
Language 6 contract that has not crossed the publication boundary.

M6 found that the improvement-request bundle's registered profile is
`mori/improvement-requests-profile.dhall`, not the nonexistent
`docs/improvement-requests/profile.dhall` named by the original closeout command. Profile and log
enforcement pass for all 39 records without `--strict`. Strict validation exposes a pre-existing
bundle-wide baseline: 11 records, including IR-39, omit the profile-recommended `reviews` field.
No review provenance was invented during implementation merely to make that recommendation pass.

The full repository gate also exposed two maintained inventories outside the focused M5 checks.
`keiro-dsl/test/Main.hs` needed the seven new Language 6 reaction/diff fixtures in its explicit
candidate-fixture allowlist and the state-authority round-trip expectation needed the fourth
`ResponderIgnored` input. After those tests passed, the record-migration policy correctly required
regenerating `generated-haskell-edition-idiomatic-v2.md` and `record-field-migration-0.15.md` for
the public record fields introduced by M1--M5.
...
- Decision: Implement IR-39 through a runtime prerequisite, [plan 279](279-harden-process-manager-reaction-apis-before-dsl-generation.md), followed by this DSL plan.
  ...
  Date: 2026-09-10
- Decision: Add `ProcessReactionSyntax` to candidate `language keiro-dsl 6` and retain its
  existing runtime profile `keiro-dsl/runtime-semantics/5`.
  Rationale: Language 5 is `Stable PublishedLanguage`; Language 6 is already the active,
  unpublished candidate for delegated inbox syntax. ADR-16 permits amending that candidate in
  place. The reaction body changes no aggregate replay or fold behavior, so ADR-18 gives no reason
  to add another `RuntimeCapability` or allocate Language 7.
  Date: 2026-09-10
...
- Decision: Preserve all existing Language 6 delegated-inbox features and baseline entries while
  adding reaction syntax to the same candidate profile.
  Rationale: Candidate correction must be additive across concurrent candidate work in the current
  tree. Replacing `profileV5`, `runtimeProfileV5`, or the existing candidate fixtures would regress
  already-delivered behavior unrelated to this plan.
  Date: 2026-09-15

- Decision: Validate IR-39 with the registered improvement-request profile and preserve the
  bundle's existing strict-validation baseline instead of adding unauthenticated review records.
  Rationale: `mori registry show shinzui/keiro --full` identifies
  `mori/improvement-requests-profile.dhall` as the owning profile. Required fields, profile
  enforcement, and log enforcement pass for all 39 records; strict mode promotes the optional
  `reviews` recommendation to an error for 11 existing records. A review entry must describe an
  actual review, not be synthesized as gate metadata.
  Date: 2026-09-16
...
Implementation restart outcome (2026-09-15): the repository advanced after the prior plan update.
Language 6 and its authoring/baseline plumbing now exist for delegated inbox syntax, but process
reaction grammar, checking, generation, and conformance remain absent. M1 therefore extends the
same unpublished candidate instead of creating another version; no runtime capability changes.

M4 outcome (2026-09-16): Language 6 reactions now scaffold executable workers, typed timers and
firers, stable metadata and decoder obligations. Three PostgreSQL suites cover guards, typed
silence, no-advance, accepted-only and partial fan-out, timer rearm/once/cancel/fire/terminal/race
and ceiling behavior. Mutation tests pin guards, payloads, commands and timer identity. The scaling
benchmark records generated bytes and scaffold time, while the opt-in isolated probe directly
separates saga hydration, target hydration and witness recovery at 8/32/128 fan-out and under a
synchronized conflict. The ordinary build emits no probe markers. M6 documentation/OKF closeout
remains.

M5 outcome (2026-09-16): reaction evolution is now classified at the input, arm ordinal, fan-out,
timer, version, and SHA-256 fingerprint surfaces. A versioned reorder is advisory, the same reorder
without a bump is breaking, and migration between positional and target-keyed dispatch identities
is breaking with the deployment-drain reference. Scaffold ledgers retain reaction coordination
metadata and record legacy bodies as `custom-unverified` without invented version or fingerprint.
The focused diff group passes 105 examples, all new fixtures pass `keiro-dsl check`, and the
same-path scratch CLI produces the expected exit codes. M6 documentation/OKF closeout remains.

Completion outcome (2026-09-16): Language 6 now makes process reactions a checked, generated,
and operationally documented DSL surface. Authors can define timer-free or multi-timer guarded
reactions; generated workers use the accepted-witness runtime, target-keyed dispatch recovery, and
typed decoder hole described by the extended ADR-41. IR-39 is completed. The final `just verify`
run exits 0: the runtime suite passes 694 examples, PGMQ passes 58 with its two existing pending
environment cases, operations passes 48, the main DSL suite passes 742 and every declared DSL
conformance suite passes, Jitsurei passes 25, and migrations pass 36. The 43-entry generated corpus,
reaction mutation proof, hydration/scaling proof, documentation bundles, record-migration policy,
and generated-name policy all pass without drift. The only non-green command considered during
closeout is the separately documented pre-existing 11-record strict IR profile recommendation;
required IR profile and log enforcement pass for all 39 records.
...
`keiro-dsl/src/Keiro/Dsl/LanguageVersion.hs` registers Languages 1 through 6; Language 5 is the
sole `Stable` `PublishedLanguage`, while Language 6 is the active authoring
`CandidateLanguage` and currently owns delegated inbox syntax. `LanguageFeature` is the closed
list of grammar gates and `SyntaxProfile` values are private, so reaction syntax must be added to
the existing unpublished Language 6 profile. Adding a feature also
...
Those files already recognize Language 6 and `candidate-primary`; M1 preserves that plumbing and
adds reaction feature coverage and suites. Skeleton updates in `keiro-dsl/src/Keiro/Dsl/Skeleton.hs`
change the Language 6 process template while leaving frozen older-language corpora untouched.
...
Confirm `LanguageVersion.hs` still has exactly one active unpublished candidate, Language 6,
before extending its syntax profile. Language 6 registration is already present; reaction feature
registration is owned by this plan, not by the runtime prerequisite.
...
`Keiro.Dsl.LanguageVersion` now contains the Language 6 candidate added after M0 for delegated
inbox syntax. Adding reaction syntax to that candidate remains Milestone 1 work.
...
In `keiro-dsl/src/Keiro/Dsl/LanguageVersion.hs` add the new constructor
`ProcessReactionSyntax` of `LanguageFeature` to the existing `profileV5`, alongside
`DelegatedInboxSyntax`. Keep the existing Language 6 registry row and `runtimeProfileV5`, and keep
Language 5 `Stable PublishedLanguage`. Update
...
the existing baseline files only to register reaction suites: they already set
`authoringLanguageVersion: 6`, `primaryLanguageVersions: [4, 5, 6]`, and enforce that a
`candidate-primary` suite names the current authoring language. Extend the `describe
"process/timer (EP-3)"` block in `keiro-dsl/test/Main.hs`
...
cp keiro-dsl/test/fixtures/process-reactions-diff.keiro "$reaction_diff_dir/reactions.keiro"
...
The improvement-request command uses the profile registered for that bundle. Adding `--strict`
currently reports the 11-record missing-`reviews` baseline described in Surprises & Discoveries;
it is recorded evidence, not a completion gate for this plan until those reviews actually exist.
...
and the existing Language 6 syntax profile.
...
2026-09-15: Began implementation and reconciled the plan with the post-`6b16d217` tree. Language 6
is already the active unpublished candidate for delegated inbox syntax, so M1 now extends its
syntax profile in place, preserves runtime profile 5 and existing baseline plumbing, and does not
allocate Language 7. Reaction implementation remains pending at this stopping point.
