...
The semantic `Spec` QuickCheck renderer is intentionally unversioned compatibility input. Its base generator must therefore keep table idempotence; candidate delegated round trips need an explicitly versioned source test. Letting the generic generator emit delegated syntax would test a Language-6 feature through the legacy Language-1 parser.
