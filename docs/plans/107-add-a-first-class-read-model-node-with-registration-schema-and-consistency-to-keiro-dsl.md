...
- [ ] `Keiro.Dsl.ReadModelShape` module: canonical shape string + FNV-1a-64 derivation,
      with unit tests pinning example digests.
- [ ] Eleven new `DiagnosticCode` constructors appended (no existing code renamed); node
      rules implemented in `keiro-dsl/src/Keiro/Dsl/Validate.hs`.
- [ ] `QueryOp` read-model + consistency resolution and `PgmqDispatch`
...
- [ ] Negative fixtures added and asserted by code (not prose) in `keiro-dsl/test/Main.hs`;
