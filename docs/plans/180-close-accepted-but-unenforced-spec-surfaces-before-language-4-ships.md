...
- [x] (2026-08-02 15:20Z) Milestone 4: language-4 semantic couplings for the
  ...
  examples.
- [x] (2026-08-02 15:31Z) Milestone 5: dead-grammar removal, negative-test
  coverage for previously untested diagnostics, documentation, and ADR
  amendments.  `cabal build all`, `cabal test all
  --test-show-details=direct`, `nix flake check`, and strict ADR validation all
  passed; the focused DSL suite passed 482 examples.
...
- Discovery (validation pass, corrected in Milestone 5): the exact emitted
  single-spec diagnostic inventory to pin in Milestone 5 is `UndeclaredEvent`, `UndeclaredState`,
  ...
  `DispatchEnqueueUnresolved`, and `RunWorkflowUnresolved`.
  `RouterReadModelUnverified` is an intentional diagnostic-registry tombstone,
  not an emitted single-spec code: ExecPlan 108 records that first-class read
  models upgraded the temporary warning to `RouterUnresolvedRef` and retained
  the constructor only for API compatibility.  Diff-only, tombstone, and consumer-binding codes absent
...
- Milestone 5 outcome: the six unreachable grammar types and their workspace
  instances are gone; all twelve previously unpinned diagnostics with emitted
  single-spec paths now have direct negative coverage, while the deliberate
  `RouterReadModelUnverified` compatibility tombstone remains untouched.  The
  language reference, changelog, ADR 0004 gate inventory, and ADR update log
  now describe the shipped contract.  The focused DSL suite passes all 482
  examples.
- Overall outcome: every audited accepted-but-unenforced surface now either
  fails at the earliest sound boundary or is explicitly documented as
  descriptive-only.  Tier A corrections apply to every language version only
  where the prior output could not work; usable acceptance changes remain
  isolated to unreleased language 4.  Generated bytes, fold fingerprints, and
  runtime behavior are unchanged, and the final repository-wide build, test,
  Nix, formatting, and ADR gates pass.
...
each of the twelve emitted single-spec diagnostic codes listed in Surprises &
Discoveries, so each
...
Revision note (2026-08-02): During Milestone 5, corrected the legacy diagnostic
inventory after tracing `RouterReadModelUnverified` to ExecPlan 108's explicit
API-compatibility decision.  The twelve diagnostics with real emitted paths are
pinned; the tombstone remains exported and deliberately has no fabricated
validator path.
