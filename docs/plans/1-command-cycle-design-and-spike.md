...
The user-visible outcome of the eventual library implementation (which this plan does *not* deliver) is the ability for an aggregate author to assemble a `SymTransducer`-based aggregate, hand it to keiro along with a codec and a snapshot policy, and call something like:
    ...
    runCommand :: EventStream a -> Stream a -> Command a -> Eff es (Either CommandError [Event a])
...
- **The transactional-step primitive** depends on a kiroku-store upstream feature that does not exist today (single-stream `appendToStream` does not open a Haskell-layer transaction). The spike does not implement `runCommandWithSql`; it is unblocked by the upstream addition. EP-3 picks it up. **[CLOSED 2026-05-10 — kiroku-store now ships `Kiroku.Store.Transaction.runTransactionAppending` (with `runTransactionAppendingNoRetry`, `appendToStreamTx`/`prepareEventsIO`, and the bare `runTransaction`/`runTransactionNoRetry` escape hatch). `runCommandWithSql` should bind directly to `runTransactionAppending` for single-stream commands; see the 2026-05-10 Revisions entry below and the parent MasterPlan's matching Revisions entry.]**
...
- No transactional read-then-append for a single stream (kiroku's `Store.Effect` only opens a Haskell-layer transaction inside `appendMultiStream`). **[CORRECTED 2026-05-10 — `Kiroku.Store.Transaction.runTransactionAppending` (and the `appendToStreamTx`/`runTransaction` siblings in the same module) now expose a single-stream transactional surface; the gap this bullet records is closed. See the 2026-05-10 Revisions entry below.]**
...
`Stream a` (defined in keiro, *not* in kiroku) is a typed wrapper carrying a `StreamName`; the `a` parameter ties it to a specific `EventStream` instance so the type system rejects "command-meant-for-Order applied to a Counter stream" mistakes. The exact phantom-vs-data-family choice is part of the M2 design doc.
...
- *Type-safe stream identity* — define `newtype Stream a = Stream StreamName`. `a` is the event-stream-identity type whose `EventStream` instance carries the matching transducer/codecs. Explain why the typed wrapper lives in keiro (not kiroku).
...
- *Tick / silent-advance entrypoint* — beyond the command-driven cycle, expose `tick :: Stream a -> Eff es ()` that loads, runs `delta` without a command (looking for an active ε-edge), and persists any silent state advance. Useful for timer-driven workflows. Discuss whether the ε-transition should produce a domain event for log fidelity or remain truly silent (preliminary recommendation: emit a `StateAdvanced` synthetic event for replay determinism).
...
- *Multi-stream commands* — design of `runCommandMulti` that takes a list of `(Stream, ExpectedVersion, command)` triples and uses `appendMultiStream`. Note the deadlock-avoidance rule (kiroku pre-locks streams in `stream_id` order — already handled by kiroku).
...
By the end of M2, `docs/research/06-command-cycle-design.md` additionally fixes the production-shape signatures including `Stream a` (named `AggregateId a` in the design doc as published 2026-05-05; renamed 2026-05-13 — see Revisions and the corrections note at the top of the design doc), the snapshot-aware variant of `runCommand`, the `tick` entry point for ε-edge advancement, `runCommandMulti`, and the transactional-step combinator `runCommandWithSql`.
...
- 2026-05-10: **Marked the kiroku transactional-step upstream gap as CLOSED.** Kiroku-store now ships `Kiroku.Store.Transaction` with `runTransactionAppending`/`runTransactionAppendingNoRetry` (the recommended high-level wrapper combining a single-stream append plus a user-supplied `(AppendResult -> Tx.Transaction a)` continuation in one ACID transaction; signature `StreamName -> ExpectedVersion -> [EventData] -> (AppendResult -> Tx.Transaction a) -> Eff es (Either StoreError a)`), `appendToStreamTx`/`prepareEventsIO`/`PreparedEvent` (the lower-level `Tx.Transaction`-flavored building block; UUIDv7 generation must happen outside the no-`MonadIO` body), and `runTransaction`/`runTransactionNoRetry` (the bare escape hatch wrapping arbitrary `Hasql.Transaction.Transaction` work against the store's pool, `ReadCommitted`/`Write`). The `Store` effect gained two new constructors (`RunTransaction`, `RunTransactionNoRetry`); a new `AppendConflict (..)` error surface with `appendConflictToStoreError` projects conflict outcomes onto `StoreError`. Live source: `/Users/shinzui/Keikaku/bokuno/kiroku-project/kiroku/kiroku-store/src/Kiroku/Store/Transaction.hs`. The implementation MasterPlan that follows EP-1 should bind `runCommandWithSql` directly to `runTransactionAppending` for single-stream commands and to `runTransaction` + `appendDispatchTx`-style assembly for multi-stream commands; the `appendMultiStream`-with-singleton-list workaround the design doc and this plan body documented is no longer required. Inline `[CLOSED]` and `[CORRECTED]` markers were added to lines 119 and 155 of this plan body to flag the affected gap-list bullets in place. The published EP-1 design doc at `docs/research/06-command-cycle-design.md` is preserved verbatim per the closed-doc convention; a top-of-file corrections note plus inline markers at §10 ("Upstream gap") and §14 ("Single-stream transactional append") record the closure overlay. The parent MasterPlan's 2026-05-10 Surprises & Discoveries entry "cross-cutting, follow-up to the EP-8 M3 discovery — full kiroku transaction API surface" and matching Revisions entry document the cascade. EP-1 status remains Complete; only doc-level cross-references are refreshed. Reason: the user shipped the kiroku transaction primitive that EP-1 §10 originated as an upstream request and asked for a comprehensive doc cascade.
...
- 2026-05-13: **Renamed the typed event-stream-id wrapper `AggregateId a` → `Stream a`** (and `unAggregateId` → `unStream`) across this plan body. The contract type was already renamed `Aggregate phi rs s ci co` → `EventStream phi rs s ci co` on 2026-05-08; the typed-id wrapper was deliberately kept as `AggregateId` then because `EventStreamId` was judged too verbose, but the user identified on 2026-05-13 that `AggregateId` is still too DDD-flavoured for keiro's general-purpose framing. The first 2026-05-13 pass selected `StreamRef a` (chosen specifically to dodge the `Streamly.Data.Stream.Stream` collision); after team feedback that `StreamRef` reads weird at the API surface, the rename was redone in the same change-window with `Stream a` as the final selection. Streamly's `Stream` collision is resolved at use sites with qualified imports (`import qualified Streamly.Data.Stream as Stream`, exposing `Stream.Stream`); only keiro modules that introduce or destructure the typed `Stream a` newtype need the qualified import — Streamly-stream-consuming code that doesn't name the typed-id can leave the import unqualified. **Updates this revision applied (this plan only)**: lines 37, 122, 232, 316, 319, 323, 440 — live `runCommand` signature in §"Vision & Scope", *Comparison against the original purpose* in §"Outcomes & Retrospective", *Type-safe stream identity* design-doc-outline bullet, *Tick / silent-advance entrypoint* outline bullet, *Multi-stream commands* outline bullet, the M2 closing summary line. The M2.1 done-note on line 55 (which describes what the design doc was when it was first written 2026-05-05) is preserved verbatim as historical record — the design doc as published does carry the `AggregateId` name, and the rename overlay is recorded by the corrections note added 2026-05-13 at the top of `docs/research/06-command-cycle-design.md`. The parent MasterPlan's 2026-05-13 Decision Log + Revisions entries document the cross-plan cascade and the alternative-comparison reasoning. EP-1 status remains Complete; only the type name is refreshed. Reason: the user observed that `AggregateId` is too tied to DDD and keiro is more general; team feedback after the StreamRef intermediate selection preferred the bare `Stream` name even at the cost of the Streamly collision (the conventional Haskell remedy of qualified imports is cheap here, since the typed-id appears only at the keiro public-API surface).
