...
- [ ] M0.1 — First-principles contract derivation: enumerate the requirements (event sourcing, workflows, snapshots, idempotency) and ask which of keiki's primitives (`SymTransducer` with `step`/`delta`/`omega`/`applyEvent`/`applyEvents`/`reconstitute`) each requirement consumes. Write the candidate contract.
- [ ] M0.2 — Sanity-check the candidate contract against the four follow-on plans (codecs, subscriptions/process-managers, snapshots, workflow roadmap) to confirm it does not block any of them.
...
- *Hydration phase* — pagination, batch sizing, the `DecodeError` and `ReplayError` cases, when an unknown-event-type is fatal vs ignorable. Note that hydration produces `(s, RegFile rs)`, not just `s`.
