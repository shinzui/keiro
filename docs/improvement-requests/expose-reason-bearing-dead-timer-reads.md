...
reviews:
  ...
  - kind: model
    ...
    effort: high
    context: >-
      Bundle refresh at Keiro `708aa215`. Plan 270 is complete (8/8 progress items checked, none
      open) and its link resolves; `completed` is the accurate status. `Keiro.Timer` exports
      `findDeadTimers` and `lookupTimerInspection`; the keiro 0.16.0.0 changelog records them
      and Hackage lists 0.16.0.0. Scope was status, link integrity, and presence of the
