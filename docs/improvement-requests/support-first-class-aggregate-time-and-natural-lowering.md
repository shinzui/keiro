...
reviews:
  ...
  - kind: model
    ...
    effort: high
    context: >-
      Bundle refresh at Keiro `708aa215`. Plan 157 is complete (8/8 progress items checked, none
      open) and its link resolves; `completed` is the accurate status. Aggregate time and
      natural lowering remain in `Keiro.Dsl.NominalType` and `Keiro.Dsl.Scaffold`. Scope was
