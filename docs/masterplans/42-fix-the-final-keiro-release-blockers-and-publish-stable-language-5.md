...
- EP-4's full DSL gate found two hidden stable-pointer couplings: released feature diagnostics
  ...
  examples then pass.
- EP-4's first repository-wide candidate gate found that a compile-time safety probe selected
  `-package keiro` by bare package name. EP-3's installed 0.11.0.1 metadata probe made that name
  ambiguous beside the current 0.11.0.0 build. Pinning the nested GHC invocation to the package
  version generated by Cabal preserves the intended type-error assertion across local package
  environments; the fresh-database full gate passes at `5975ee147a3e4c00fb8eff097bd6c4a2b3add36b`.
- EP-4's first adversarial follow-up sweep found a stale worked-guide constraint, obsolete
