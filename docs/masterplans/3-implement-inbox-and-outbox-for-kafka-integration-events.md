...
| 23 | Add a Shibuya inbox adapter for two-stage integration-event processing | docs/plans/23-add-a-shibuya-inbox-adapter-for-two-stage-integration-event-processing.md | EP-21 | EP-16, EP-20, EP-22 | Complete |
...
- [x] EP-23: optional Shibuya inbox adapter for two-stage drain. (2026-05-19) — `Keiro.Inbox.Adapter` polls `keiro_inbox` under `FOR UPDATE SKIP LOCKED`, hands rows to a Shibuya `Handler`, and translates `AckDecision` into inbox state. The inline EP-21 `runInboxTransaction` path remains the default; the adapter is purely additive.
...
- 2026-05-19: EP-21's inline `runInboxTransaction` is correct but creates two real operational gaps: (a) handler latency lives on the Kafka consumer's critical path, so a slow handler stalls the partition for every consumer in the group; (b) the inbox has only one writer — the Kafka consumer — so a saga that wants to record a synthetic integration receipt has nowhere to write a `pending` row. EP-23 closed both gaps by adding `Keiro.Inbox.Adapter` (work-side drain) and `enqueueInbox` / `enqueueInboxTx` (multi-writer entry points) without removing the inline path.
- 2026-05-19: Hasql's `runTransaction` does not surface `Tx.condemn` status to the caller — a condemned transaction returns the body's value just like a committed one. This bit `mkTransactionalInboxHandler` design: the wrapper cannot detect "user rolled back" via the return value alone. EP-23 chose the explicit `Tx.Transaction (Either Text a)` signature so the rollback signal is a first-class part of the contract. A future refinement could expose a "commit status" runner from `Kiroku.Store.Transaction` and let the wrapper accept the simpler `Tx.Transaction a` shape again.
- 2026-05-19: `codd`'s `embedDir`-based migration loader does not invalidate the Template Haskell splice when a new SQL file is added under `sql-migrations/`. Cabal sees no `.hs` change and reuses the cached `Keiro.Migrations.o`, so `runAllKeiroMigrations` quietly reports "N found" instead of "N+1". Worked around by deleting the cached `.o` / `.dyn_o` files (or `touch`ing the module) when adding migrations. Worth surfacing in the keiro-migrations README and in EP-16's onboarding notes.
...
- Decision: Ship the **inline `runInboxTransaction`** and **drained `Keiro.Inbox.Adapter`** paths together as supported alternatives, picked by the consumer per integration. EP-21 remains the default for handlers that fit in one Postgres transaction; EP-23 is the right tool when handler latency would block Kafka, when multiple producers write the inbox, or when a Shibuya supervisor should own both the Kafka receive and the handler work.
  Rationale: Each path has a regime where it is strictly better. The inline path's single-transaction `(insert + handler + markCompleted)` is the strongest atomicity available; removing it would force every caller onto looser semantics for a benefit they may not want. The drained path is the only way to decouple consumer lag from handler latency and the only way to compose Kafka receive with sagas/HTTP shims under one inbox. Picking only one would force a trade-off the design does not need to make. `mkTransactionalInboxHandler` covers the cases where a drained-path user still wants inline atomicity for a single Postgres tx.
  Date: 2026-05-19.
...
- 2026-05-19: Added EP-23 to the registry and progress. Closed the inbox-side gaps surfaced by EP-21's inline-only design (consumer lag pressure and single-writer constraint) by adding `Keiro.Inbox.Adapter` plus the new public `enqueueInbox` / `enqueueInboxTx` API. Recorded the `Tx.condemn`-vs-`Either` decision and the inline-vs-drained coexistence decision; documented both in `docs/guides/integration-events-with-kafka.md`.
