...
- 2026-05-19: Hasql's `runTransaction` does not surface `Tx.condemn` status to the caller — a condemned transaction returns the body's value just like a committed one. Surfaced while building (and then reverting) EP-23, where a wrapper that needed to know whether the user-supplied transaction rolled back could not learn it from the return value alone. Any future "drain the inbox with a separate worker" design must either ship its own commit-status runner from `Kiroku.Store.Transaction` or accept an explicit `Either`-style rollback signal in the user contract.
- 2026-05-19: `shibuya-kafka-adapter`'s `consumerRecordToEnvelope` only preserves `traceparent`/`tracestate` headers; the rest of the Kafka headers (including the keiro-* headers the inbox needs to reconstruct an `IntegrationEvent`) are dropped at the adapter boundary. Today there is no turn-key way to take a `Shibuya.Adapter.Adapter es (Maybe ByteString)` from `shibuya-kafka-adapter` and feed it straight into `runInboxTransaction`. Two paths: extend `shibuya-kafka-adapter`'s envelope to carry the full header set (the right upstream fix), or use `kafka-effectful` directly for keiro-inbox consumers. Documented in `docs/guides/integration-events-with-kafka.md`.
...
- 2026-05-19: Attempted and reverted EP-23 (a Shibuya inbox adapter for two-stage drain). The full implementation was built across five commits (`35422b6`, `0f69e4c`, `8345140`, `277c6ca`, `5219561`) with 14 new tests green, then reverted in a single revert commit on the same day. Reasons: no concrete near-term consumer needed the drained path; `mkTransactionalInboxHandler`'s `Either Text a` workaround for `Tx.condemn` non-observability revealed a leaky abstraction; the "atomicity restored" framing of the wrapper was overstated; and the schema additions were a one-way door without a real driver. See `docs/plans/23-...` Outcomes & Retrospective for the full reasoning. Two findings from the attempt outlive the revert and are recorded above: Hasql's `Tx.condemn` non-observability and `shibuya-kafka-adapter`'s drop of keiro-* headers in `consumerRecordToEnvelope`. Both are documented in the integration-events guide and this MasterPlan's Surprises & Discoveries.
