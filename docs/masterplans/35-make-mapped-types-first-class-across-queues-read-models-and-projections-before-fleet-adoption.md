...
| 5 | Extend semantic impact conformance and evolution reporting to every mapped surface | [Plan 227](../plans/227-extend-semantic-impact-conformance-and-evolution-reporting-to-every-mapped-surface.md) | EP-2, EP-3, EP-4 | None | Complete |
| 6 | Qualify the complete mapped consumer surface before fleet adoption | [Plan 228](../plans/228-qualify-the-complete-mapped-consumer-surface-before-fleet-adoption.md) | EP-5 | None | Complete |
...
- [x] EP-5: integrate every root into semantic impact, coverage, reports, ledgers, and conformance.
- [x] EP-5: mutation-test independent queue, query, projection, event, and snapshot classifications.
- [x] EP-6: pass cross-surface exact-tree, compatibility, and restoring-mutation qualification.
- [x] EP-6: refresh the candidate corpus once, publish adoption guidance, and close both gates.
...
- Empty current semantic-impact inventories require an explicit version marker. Per-declaration
  evidence cannot distinguish an empty current graph from a legacy aggregate-only baseline when
  there are no declaration rows to carry the new fields.
- Service-facade surface facts must be feature-gated with candidate language 5. Applying them to
  stable language 4 would change published generated bytes even though the underlying aggregate
  mapping semantics were unchanged.
- Exact locality follows generated role, not a blanket declaration-consumer rule. Structural queue
  edits do not churn projection witnesses, while register mapping identity reaches the aggregate
  transducer because it participates in snapshot identity. Reviewed path allowlists now encode
  those distinctions and remain constant after unrelated workspace growth.
- The complete repository run exposed an arbitrary-test generator that could construct a
  non-catalog read model with no physical table/schema authority. Constraining the generator to
  grammar-spellable physical names made the existing parse/pretty property honest without changing
  the language or runtime contract.
...
- Decision: Use a versioned current semantic-impact marker and gate new facade facts on the
  candidate mapped-consumer feature.
  Rationale: The marker preserves exact known-empty history, while the language gate keeps
  published generated output immutable and places the new conformance contract only where its
  source syntax is owned.
  Date: 2026-08-10
...
EP-5 is complete. One semantic authority now projects every explicit queue/query/aggregate root,
derived projection consumer, complete logical path, operational projection identity, and
orthogonal build/history/API/rebuild consequence. Coverage, diff text/JSON, scaffold reports,
candidate service-conformance facts, and version-marked standalone/workspace ledgers consume that
authority. Languages 1–4 retain their generated Haskell bytes. Full build and the 660-example DSL
driver pass, all compiled conformance components pass, restoring structural/queue/read-model
mutations pass, and the 37-entry corpus replay reports record/disk and Cabal inventory consistency.
EP-6 is now dependency-ready.

EP-6 is complete. The integrated candidate service compiles and executes event, snapshot, queue,
query, inline-projection, catalog-projection, structural, opaque, legacy, and unsupported
boundaries together. Production semantic authority drives exact consequence and generated-tree
expectations in both standalone and workspace forms; adding an unrelated service member does not
amplify an existing edit. Ten restoring corruptions fail at their named gate and restore exact
authority bytes. The reviewed candidate corpus, 664-example DSL driver, complete Cabal build/test
matrix, ADR/improvement-request checks, formatter, flake checks, and diff hygiene pass.

Both MP-34 and MP-35 repository prerequisites are now complete. The gate permits a separately
authorized fleet-adoption project to begin, but this initiative performed no package release,
downstream rewrite, queue drain, projection rebuild, or application SQL migration. The published
adoption guide preserves those operational decisions as explicit service-owner work rather than
claiming the DSL performed them.
