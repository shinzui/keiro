...
- Decision: EP-2 adopts the EP-1 fork pin in `kafka-effectful` rather than deferring adoption, even though EP-2's own tests do not require it.
  Rationale: This MasterPlan's Integration Points said EP-1 owns the pin recipe and consumers adopt on next build. That reading turned out to be too weak for this consumer: the whole point of EP-2's classifier is that a fatal error throws, and without the pin the fatal is discarded inside hw-kafka-client's background loop before any Haskell code runs, so in the default async poll mode `classifyPollError` can never see it. Shipping a classifier that cannot observe the condition it exists to classify would reproduce the original defect one layer up. `shibuya-kafka-adapter` and `shikigami` remain unpinned and keep async-mode fatal blindness until they adopt it.
  Date: 2026-07-27
