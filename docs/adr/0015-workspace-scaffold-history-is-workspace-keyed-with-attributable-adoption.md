...
For every checked mapped declaration it records the sorted aggregate consumers,
service-inventory membership, and a canonical declaration identity built from
resolved metadata, wire identity, and generated Haskell-facing names. Source
...
- A mapped declaration edit can explain old and current aggregate consumers even
  when the consumers themselves did not change. A source-only move can report a
