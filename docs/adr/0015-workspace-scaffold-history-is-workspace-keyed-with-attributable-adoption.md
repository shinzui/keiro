...
**Idempotence is observable.** Both standalone and workspace write paths compare
bytes before overwriting a Generated module and report `unchanged` without
writing when they match. A re-run with no source edits therefore proves it
changed nothing, rather than asserting it.

**Mapped semantic history is source-independent and explicit.** Every successful
standalone or workspace ledger writes one additive `semantic-impact` JSON row.
For every checked mapped declaration it records the sorted typed consumers,
complete surface/root paths, orthogonal compatibility consequences, service-inventory
membership, and a canonical declaration identity built from resolved metadata,
wire identity, and generated Haskell-facing names. Projection evidence additionally
records its group, targets, observing read models, and replay policy. Source
locations, member ownership, and declaration order are excluded, so source-only
movement cannot manufacture semantic impact. The identity is necessary because
consumer sets alone do not reveal a same-consumer wire-shape change.

An absent row means the earlier ledger predates this evidence and is reported as
an unavailable baseline, never as an empty old graph. Current-ledger reports
compare the two checked snapshots; legacy-ledger reports name only current
consumers for declarations already made visible by older mapping-drift evidence.
Semantic impact is rendered separately from actual generated-artifact
dispositions. Aggregate modules, the context `StructuralConformance` module, and
the context `BehaviorSourceMap` module keep distinct roles; a changed file is
evidence of generation, not dependency authority.
...
whose ordered invocations share one record slot tracks only the final authoritative pair;
intermediate or nested stale pairs are not histories.
...
- A mapped declaration or consumer-relation edit can explain old and current
  aggregate, queue, query-position, and projection consumers even when the
  declaration bytes did not change. A source-only move can report a
  changed behavior-source-map artifact while reporting no mapped semantic impact.
- The first accepted scaffold after a pre-feature ledger establishes a semantic
  baseline; no removed-consumer claim is made before that baseline exists.
- The repository's committed conformance corpus is reproducible from a clean checkout:
  its tracked records supply ordinary invocation provenance, while the global ignore rule
  still prevents application-local scaffold metadata from appearing as untracked noise.
