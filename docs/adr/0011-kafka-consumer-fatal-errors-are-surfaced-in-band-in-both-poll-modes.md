...
description: A raised librdkafka consumer fatal is reported as Left RdKafkaRespErrFatal from every poll in both callback poll modes, via a pinned hw-kafka-client fork.
...
`hw-kafka-streamly`, `kafka-effectful`, and `shibuya-kafka-adapter`. `kafka-effectful` has
no `cabal.project` today and needs one created to adopt it.

Any code that classifies Kafka errors must treat `RdKafkaRespErrFatal` as fatal and must
not filter it out. `hw-kafka-streamly`'s `isFatal` does; so must the `kafka-effectful`
...
under `docs/masterplans/23-make-the-kafka-consumer-streaming-stack-surface-fatal-errors-and-close-deterministically.md`.
Two sibling plans extend it and this record should be revised when they land:
`docs/plans/136-classify-poll-and-commit-errors-and-fix-traced-context-hygiene.md` adds the
rule for routine in-band poll conditions and idle commits — which must *not* be treated as
fatal — and `docs/plans/137-guarantee-deterministic-consumer-close-in-hw-kafka-streamly.md`
adds deterministic close, without which a stream that observes a fatal may still leave the
dead consumer open until garbage collection runs.
