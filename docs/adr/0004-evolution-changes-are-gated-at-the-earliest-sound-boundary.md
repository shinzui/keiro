...
| Guard tightening or emitting-body removal | Replay-only edge discipline from ADR 0002 | Exact family remainders select replay bodies, whose complete old/new live guard unions classify preservation and removal. Same-body coverage must cover the exact region or every old alternative. A proposed twin is advertised as `AggGuardTightened` only after effective-language insertion, render, parse, identity, outcome-clearing, and validation proofs; failure is `AggGuardRemedyUnavailable` with no transition block. Explicit Hole-owned changes are `AggGuardRelationUnknown`. All three are private-history advisories, and `diff --deny CODE` can promote any code to a failing invocation | Source proof does not replace generated conformance or runtime validation. Targeted real-log audit fails without a required twin and passes with it; unknown or unavailable remedies must be resolved or audited before deployment |
