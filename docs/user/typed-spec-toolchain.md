...
graph and names the declaration's previous and current aggregate consumers plus
the one service-wide structural-conformance role. Artifact impact reports which
...
the checked old/current aggregate consumers and the service conformance role;
it does not replace the detailed compatibility findings, rollout constraints,
or replay report. `--report-out` appends the same sorted projection under
