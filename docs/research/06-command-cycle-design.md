...
> **Corrections note (2026-05-10).** §10's *Upstream gap* paragraph and §14's "kiroku-side / Single-stream transactional append" entry **have been satisfied**: kiroku-store now ships `Kiroku.Store.Transaction` exposing `runTransaction`/`runTransactionNoRetry` (bare escape hatch over `Hasql.Transaction.Transaction`), `appendToStreamTx` (`Tx.Transaction`-flavored single-stream append building block paired with `prepareEventsIO` for UUIDv7 generation), and **`runTransactionAppending`/`runTransactionAppendingNoRetry`** — the recommended high-level wrapper combining a single-stream append plus a user-supplied `(AppendResult -> Tx.Transaction a)` continuation in one ACID transaction. Live source: `/Users/shinzui/Keikaku/bokuno/kiroku-project/kiroku/kiroku-store/src/Kiroku/Store/Transaction.hs`. **Cascade for this document**: §10's `runCommandWithSql` should bind directly to `runTransactionAppending` for single-stream commands and to `runTransaction` + `appendDispatchTx`-style assembly for multi-stream commands; the `appendMultiStream`-with-singleton-list workaround §10 documents is no longer required. EP-8's spike at `spikes/read-model/src/Spike/Command.hs` (lines 23 and 130) is the first in-tree consumer of the wrapper. **This document is preserved verbatim below for traceability** (per the closed-doc convention established for documents 04 and 05); inline `[CLOSED]` markers flag the two passages where the upstream gap is named in the body. The parent MasterPlan's 2026-05-10 Surprises & Discoveries entry "cross-cutting, follow-up to the EP-8 M3 discovery — full kiroku transaction API surface" and matching Revisions entry record the consolidation pass that produced this corrections note.

> **Corrections note (2026-05-13) — typed event-stream-id rename.** The typed wrapper §3 introduces as `newtype AggregateId a = AggregateId { unAggregateId :: StreamName }` (and references throughout as `AggregateId a` in `runCommand`, `runCommandRetry`, `tick`, `runCommandMulti`, `runCommandWithSql`, the `SomeMultiCommand`/`SomeEvent` packagings, the OpenTelemetry attribute description, and the spike-vs-production-library contrast on line 459) **has been renamed** `AggregateId a` → `Stream a` (with `unAggregateId` → `unStream`) across the keiro research foundation. The user identified on 2026-05-13 that `AggregateId` is too tied to DDD: keiro v1 process managers (`pm:…` streams), v2 workflows (`wf:…` streams), and read models are *all* consumers of the typed wrapper but are not aggregates in the DDD sense, so the wrapper's name should reflect what it actually is — a typed event-stream identifier. Considered alternatives were `StreamRef a` (selected by the first 2026-05-13 rename pass to dodge the Streamly collision, then rejected after team feedback that `StreamRef` reads weird at the API surface), `StreamId a` (conflicts with kiroku's surrogate `StreamId :: Int64`), `EventStreamId a` (rejected on 2026-05-08 as too verbose, judgement still holds), and `EsId a` (too cryptic for a public-API-facing type). Final selection: `Stream a`, accepting the name collision with `Streamly.Data.Stream.Stream` and resolving it the conventional Haskell way — keiro modules that introduce or destructure the typed `Stream a` newtype must `import qualified Streamly.Data.Stream as Stream`; modules that consume Streamly streams without naming the typed-id can leave the import unqualified, since the typed-id only appears at the keiro public-API surface. The rename is uniform across every body mention in this document and across `01-…`, `08-…`, `11-…` (see their corrections notes). The keiro-side type substance is unchanged: same single-field `newtype` over `StreamName`, same phantom `a` ties an instance to its `EventStream`, same lookup story via the (still-optional) `HasEventStream` class — only the type *name* changed. **This document is preserved verbatim below for traceability** (per the closed-doc convention established for documents 04 and 05 and reaffirmed by the 2026-05-10 corrections note above); no inline `[RENAMED]` markers are added because the rename is uniform across every body mention and would be needlessly noisy. Readers should mentally substitute `Stream`/`unStream` for any body mention of `AggregateId`/`unAggregateId`. The parent MasterPlan's 2026-05-13 Decision Log + Revisions entries record the cross-plan cascade, the alternative-comparison reasoning, and the qualified-import collision-resolution convention. EP-1's plan body (`docs/plans/1-command-cycle-design-and-spike.md`) updates its live references in place; only this design doc is preserved verbatim per the closed-doc convention.

This document fixes the contract and the runtime cycle that keiro will expose for handling a command targeted at an aggregate. It is the foundation document every other keiro design plan refers to. The original accompanying spike at `spikes/command-cycle/` was the empirical proof that the contract is implementable on the existing kiroku and keiki primitives, including under concurrent writers.

> **Retirement note (2026-05-19).** The spike at `spikes/command-cycle/` has been removed. Its design has been absorbed into the live keiro library — `runCommand`, `runCommandWithSql`, `runCommandWithSqlEvents`, and the surrounding `CommandError`/`RunCommandOptions` surface now live in `src/Keiro/Command.hs`, the typed `Stream a` wrapper in `src/Keiro/Stream.hs`, and the `EventStream` value record (with `esEventCodec :: Codec co` replacing the spike's bare encode/decode pair) in `src/Keiro/EventStream.hs`. Behaviour — including the optimistic-retry path under concurrent writers — is exercised by the `Keiro.Command` group at `test/Main.hs:180`. Body references to `spikes/command-cycle/src/...` and `Spike/Retry.hs` below point at code that no longer exists; the named types and functions all live in `Keiro.Command` / `Keiro.EventStream` / `Keiro.Stream` now.
...
**Upstream gap.** **[CLOSED 2026-05-10 — kiroku-store now ships `Kiroku.Store.Transaction.runTransactionAppending` (and the `appendToStreamTx` building block, and the bare `runTransaction` escape hatch). `runCommandWithSql` binds directly to the high-level wrapper for single-stream commands; the workaround paragraph below is preserved verbatim for traceability. See corrections note at the top of this file.]** kiroku-store currently does not expose a Haskell-layer transaction primitive for *single-stream* appends — only `appendMultiStream` opens a `TxSessions.transaction ReadCommitted Write` block. Single-stream `appendToStream` runs as a single SQL CTE with no Haskell-layer transaction wrapping it, which means it cannot be combined atomically with another `Hasql.Session.Session ()`. EP-6 records this as an upstream feature request: kiroku-store needs a public combinator like
...
- *Single-stream transactional append.* Currently only `appendMultiStream` opens a Haskell-layer transaction; `appendToStream` runs as a single CTE. keiro needs a public combinator that wraps a single-stream `appendToStream` plus a user-supplied `Hasql.Transaction.Transaction a` so the transactional-step primitive (§10) can be implemented cleanly. Workaround until then: route single-stream commands through `appendMultiStream` with a singleton list. **[CLOSED 2026-05-10 — shipped as `Kiroku.Store.Transaction.runTransactionAppending` (with `runTransactionAppendingNoRetry`, `appendToStreamTx`/`prepareEventsIO`, and the bare `runTransaction`/`runTransactionNoRetry` escape hatch in the same module). The singleton-list workaround is no longer required. See corrections note at the top of this file.]**

- *Streamly-native single-stream forward read.* Today `readStreamForward` returns `Vector RecordedEvent` (a paginated read). keiro's hydration paginates inside `Stream.unfoldrM` to flatten into a `Stream`. Native streamly support would eliminate the manual paging and let keiro express hydration as a one-line `Stream.fold` over a kiroku-sourced stream. The kiroku-side `Subscription.Stream` already has the right shape — extending it to forward single-stream reads is a copy-and-modify. **[CLOSED 2026-05-14 — shipped as `Kiroku.Store.Read.readStreamForwardStream :: StreamName -> StreamVersion -> Int32 -> Stream (Eff es) RecordedEvent` (`kiroku-store/src/Kiroku/Store/Read.hs:59–75`, re-exported from `Kiroku.Store`). Internally paginates via `Stream.unfoldrM`, sharing SQL path and error semantics with the `Vector`-returning `readStreamForward`. The wrapper §5's `hydrationStream` builds at the keiro layer can now be retired in favour of the upstream primitive. See `docs/research/11-upstream-roadmap.md` 2026-05-14 corrections note.]**
...
- *Structured error model on `step`/`omega`.* Today these return `Maybe`; keiro must distinguish "guard failed" from "no edge for this command in this state" from "edge update failed" (the latter is currently impossible because updates are total, but the v2 register-typed updates may not be). A typed error sum (`Either StepError ...`) at `step`'s return site would let keiro raise more specific `CommandError` variants. **[Status 2026-05-14: still open. See `docs/research/11-upstream-roadmap.md` §7.3.]**
...
- *Register-file serialization helper.* `RegFile rs` is a typed heterogeneous tuple of `(Symbol, Type)` slots. EP-4's snapshot path needs a `StateCodec (s, RegFile rs)` that walks the slot list and serializes each. keiki could expose a `Generic`-style helper that produces a default JSON codec for any `RegFile rs` whose slot types all have `ToJSON`/`FromJSON`. Without it every aggregate author writes a hand-rolled `RegFile` encoder. **[CLOSED 2026-05-14 — shipped as the new sibling package `keiki-codec-json` (v0.1.0.0): `Keiki.Codec.JSON.regFileToJSON`/`regFileFromJSON`/`regFileToEncoding` plus TH derivation `deriveRegFileCodec`. The companion shape hash (`Keiki.Shape.regFileShapeHash` in core keiki) covers the snapshot-discriminant half of EP-4's needs. Keiro-side integration tracked by EP-9 (`docs/plans/9-integrate-keiki-codec-json-into-keiro-snapshot-path.md`), queued waiting on EP-37 Hackage upload.]**
