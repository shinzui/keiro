...
timestamp: "2026-05-09T03:29:19Z"
researchId: RES-6
status: active
...
4. **Adopt Chassaing's decider** as the keiki API surface (decide and evolve, both pure, with explicit initial state). Keiki's SymTransducer is a richer form; the Decider facade is the public API. **[RESCINDED — see corrections note above. `Keiki.Decider` is a legacy compatibility facade for the old system that keiro is replacing — it masks ε-edges, ignores the register file `RegFile rs`, and discards the symbolic predicate carrier `phi`. Keiro consumes the native `SymTransducer phi rs s ci co` directly via EP-1's `EventStream phi rs s ci co` contract. Authoritative reference: `docs/research/06-command-cycle-design.md`; `docs/masterplans/1-keiro-research-foundation.md` Decision Log entry of 2026-05-04 ("Reject `Keiki.Decider` as the keiro ⇄ keiki contract"); `docs/plans/1-command-cycle-design-and-spike.md` §M0 ("First-principles contract derivation").]**
