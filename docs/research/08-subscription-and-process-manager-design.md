...
> **Corrections note (2026-05-10).** One of the two upstream gaps this design forwards to EP-6 — *kiroku-store: single-stream `runInTransaction` combinator* — **has shipped upstream**. Kiroku-store now exposes `Kiroku.Store.Transaction` with `runTransaction`/`runTransactionNoRetry` (bare escape hatch over `Hasql.Transaction.Transaction`), `appendToStreamTx` (a `Tx.Transaction`-flavored single-stream append building block paired with `prepareEventsIO` for UUIDv7 generation outside the no-`MonadIO` body), and **`runTransactionAppending`/`runTransactionAppendingNoRetry`** — the recommended high-level wrapper combining a single-stream append plus a user-supplied `(AppendResult -> Tx.Transaction a)` continuation in one ACID transaction. Live source: `/Users/shinzui/Keikaku/bokuno/kiroku-project/kiroku/kiroku-store/src/Kiroku/Store/Transaction.hs`. **Cascade for this document**: every passage that names the *workaround* — "must route through `appendMultiStream` with a singleton list" — is superseded; the inline-projection lifecycle (§2), the outbox write path (§6), and the §13 "kiroku-store: Single-stream `runInTransaction`" gap forward should bind directly to `runTransactionAppending` instead. EP-8's spike at `spikes/read-model/src/Spike/Command.hs` (lines 23 and 130) is the first in-tree consumer of the wrapper; it implements EP-3's inline-projection lifecycle as a direct call to `runTransactionAppending`. The *other* upstream gap this design names — *shibuya-kiroku-adapter: `HandlerInTransaction` shape* (the EP-3 §1 framing and §13's second forward) — is **still open** as of this corrections note; async projections remain at-least-once with user-side idempotency until that lands. **This document is preserved verbatim below for traceability** (per the closed-doc convention established for documents 04 and 05); inline `[CLOSED]` markers flag the four passages that name the kiroku-store transaction gap. The parent MasterPlan's 2026-05-10 Surprises & Discoveries entry "cross-cutting, follow-up to the EP-8 M3 discovery — full kiroku transaction API surface" and matching Revisions entry record the consolidation pass that produced this corrections note.

> **Corrections note (2026-05-13) — typed event-stream-id rename.** This document references the typed wrapper `AggregateId a` (defined in EP-1's design at `docs/research/06-command-cycle-design.md` §3) as the second argument of `runCommand`-shaped signatures in §1 and §6 (the body's three `AggregateId a` mention sites — the inline-projection lifecycle's runCommand wrapping, the live-projection variant, and the process-manager command-handler signature). The 2026-05-08 contract-type rename `Aggregate phi rs s ci co` → `EventStream phi rs s ci co` deliberately *kept* `AggregateId` for the typed wrapper, but on **2026-05-13 the typed wrapper was renamed `AggregateId a` → `Stream a`** (with `unAggregateId` → `unStream`) across the keiro research foundation, in response to the user's observation that `AggregateId` is too tied to DDD: keiro v1 process managers (`pm:…` streams), v2 workflows (`wf:…` streams), and read models are all consumers of the typed wrapper but are not aggregates in the DDD sense. The first 2026-05-13 rename pass selected `StreamRef a` to dodge the `Streamly.Data.Stream.Stream` collision; after team feedback that `StreamRef` reads weird at the API surface, the rename was redone with `Stream a` as the final selection, accepting the Streamly collision and resolving it at use sites with qualified imports. **Streamly-collision resolution for EP-3's projection modules**: when the implementation MasterPlan ships the projection / process-manager modules described here, modules that name the typed `Stream a` newtype on their public surface (the inline-projection runCommand wrapper, the live-projection variant, the PM command handler) must `import qualified Streamly.Data.Stream as Stream` if they also consume Streamly streams (async projection runners do, via shibuya-kiroku-adapter's stream sources). **This document is preserved verbatim below for traceability**; readers should mentally substitute `Stream`/`unStream` for any body mention of `AggregateId`/`unAggregateId`. The substance of EP-3's design is unchanged — process-manager identity, command-handler signatures, and PM-as-event-sourced-aggregate framing are all stable; only the type *name* of the typed wrapper changed. The parent MasterPlan's 2026-05-13 Decision Log + Revisions entries record the cross-plan cascade and the alternative-comparison reasoning.
...
**Upstream dependency (forwarded to EP-6).** `runCommandWithSql` itself depends on a kiroku-store combinator that does not yet exist: a single-stream `appendToStream` that opens a `TxSessions.transaction ReadCommitted Write` block and accepts a user-supplied `Hasql.Transaction.Transaction a` to commit in the same tx. EP-1 §10 records this as the original request; EP-3 reinforces it (the inline-projection lifecycle is the second use case after the outbox §6). **[CLOSED 2026-05-10 — shipped as `Kiroku.Store.Transaction.runTransactionAppending` with the matching no-retry sibling and the `appendToStreamTx` building block; see corrections note at the top of this file. Implementation MasterPlan binds `runCommandWithSql` directly to `runTransactionAppending` for single-stream commands.]**
...
- *Single-stream `runInTransaction`.* (Reiterated from EP-1 §14.) `appendToStream` does not open a Haskell-layer transaction; only `appendMultiStream` does. A clean implementation of `runCommandWithSql` (and therefore §2's inline projection lifecycle and §6's outbox write path) requires kiroku to expose a public combinator that wraps `appendToStream` plus a user-supplied `Hasql.Transaction.Transaction a` in one tx. EP-3 reinforces EP-1's request. **[CLOSED 2026-05-10 — shipped as `Kiroku.Store.Transaction.runTransactionAppending`/`runTransactionAppendingNoRetry`; the lower-level `appendToStreamTx` building block and the bare `runTransaction` escape hatch ship in the same module. See corrections note at the top of this file.]**
...
Two upstream gaps block the most aggressive version of the design (exactly-once async projections via transactional checkpoint advance): kiroku-store needs a single-stream `runInTransaction` combinator **[CLOSED 2026-05-10 — shipped as `Kiroku.Store.Transaction.runTransactionAppending`; inline projections no longer rely on the `appendMultiStream`-with-singleton-list workaround. See corrections note at the top of this file.]** and shibuya-kiroku-adapter needs a `HandlerInTransaction` shape **[still open as of 2026-05-10]**. EP-6 records both; in the meantime async projections are at-least-once with user-side idempotency, and inline projections rely on `appendMultiStream` as a workaround.
