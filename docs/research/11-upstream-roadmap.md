...
> **Corrections note (2026-05-10).** The SOLE Blocking item this synthesis identified — §4.1 *kiroku-store: Single-stream `runInTransaction` combinator* — **has shipped upstream**. Kiroku-store now exposes `Kiroku.Store.Transaction` with three ergonomic levels: `runTransaction`/`runTransactionNoRetry` (bare escape hatch wrapping an arbitrary `Hasql.Transaction.Transaction a` against the store's pool, `ReadCommitted`/`Write`), `appendToStreamTx` (a `Tx.Transaction`-flavored single-stream append building block paired with `prepareEventsIO` for UUIDv7 generation outside the no-`MonadIO` body), and **`runTransactionAppending`/`runTransactionAppendingNoRetry`** — the recommended high-level wrapper combining a single-stream append plus a user-supplied `(AppendResult -> Tx.Transaction a)` continuation in one ACID transaction. Three commits delivered the surface: `2b899cb feat(store): add RunTransaction effect constructor and runTransaction smart constructor`, `2e3aeaf feat(store): add appendToStreamTx and AppendConflict for Tx-flavored single-stream appends`, `12a1611 feat(store): add runTransactionAppending convenience wrapper`. The `Store` effect gains two new constructors (`RunTransaction`, `RunTransactionNoRetry`); a new `AppendConflict` error surface with `appendConflictToStoreError` projects conflict outcomes onto `StoreError`. Live source: `/Users/shinzui/Keikaku/bokuno/kiroku-project/kiroku/kiroku-store/src/Kiroku/Store/Transaction.hs`. **Cascade**: §3's *Block 1 — Hard prerequisites for keiro v1 implementation* is now empty; the four-block sequencing is reduced to *Block 2 → Block 3 → Block 4* with no Block-1 gate. The implementation MasterPlan that follows the research foundation is therefore not gated on any upstream Block-1 item; every remaining roadmap entry is Wanted/Optional and parallelisable with v1 implementation. EP-1's `runCommandWithSql` (in `docs/research/06-command-cycle-design.md` §10) should bind directly to `runTransactionAppending` (single-stream commands) and to `runTransaction` + `appendDispatchTx`-style multi-stream assembly (multi-stream commands), retiring the `appendMultiStream`-with-singleton-list workaround §4.1's *Why* paragraph names. EP-3's inline-projection lifecycle (`docs/research/08-…` §2/§6/§13) and outbox write path can do the same. EP-8's spike (`spikes/read-model/src/Spike/Command.hs`) is the first in-tree consumer. **This document is preserved verbatim below for traceability** (per the closed-doc convention established for documents 04 and 05); inline `[CLOSED — see corrections note above]` markers flag the four passages that name §4.1 as a Blocking gate. The parent MasterPlan's 2026-05-10 Surprises & Discoveries entry "cross-cutting, follow-up to the EP-8 M3 discovery — full kiroku transaction API surface" and the matching Revisions entry record the consolidation pass that produced this corrections note.

> **Corrections note (2026-05-13) — keiro-side typed-id rename.** This roadmap's body discusses kiroku's `StreamName` opacity and the keiro-side typed wrapper that addresses it (the *What is missing today* / *Verdict* / *Provenance* triplet on lines 555-562). The keiro-side type was originally introduced as `newtype AggregateId a = AggregateId { unAggregateId :: StreamName }` (defined in `docs/research/06-command-cycle-design.md` §3) and is named that way throughout this body. **On 2026-05-13 it was renamed `AggregateId a` → `Stream a`** (with `unAggregateId` → `unStream`) across the keiro research foundation, in response to the user's observation that `AggregateId` is too tied to DDD and keiro is a more general framework. An intermediate `StreamRef a` selection from the first 2026-05-13 pass was discarded after team feedback in favour of the bare `Stream a`, accepting the name collision with `Streamly.Data.Stream.Stream` and resolving it at use sites with qualified imports (`import qualified Streamly.Data.Stream as Stream`). The substance of EP-6's verdict — "keep the typed wrapper at the keiro layer for v1; do not push `newtype StreamName a` upstream to kiroku" — is unchanged; the typed wrapper is still a keiro concept that lives at the keiro layer, and its phantom-type lookup story via `HasEventStream` is unchanged. Only the type *name* changed. **This document is preserved verbatim below for traceability** (per the closed-doc convention reaffirmed by the 2026-05-10 corrections note above); readers should mentally substitute `Stream`/`unStream` for any body mention of `AggregateId`/`unAggregateId`. The parent MasterPlan's 2026-05-13 Decision Log + Revisions entries record the cross-plan cascade and the alternative-comparison reasoning (final selection: `Stream a` over `StreamRef a`, `StreamId a`, `EventStreamId a`, `EsId a`). The kiroku-side `StreamId :: Int64` (the database surrogate) is unaffected.

> **Corrections note (2026-05-14) — second wave of upstream closures.** Six additional roadmap items have shipped since the 2026-05-10 closure of §4.1, covering both kiroku-store and keiki. The four-block sequencing in §3 is now substantially smaller than the body describes, and the dispositions in §§4.2, 4.6, 4.7, 4.9, 7.1, 7.2 are stale. Inline `[CLOSED — see corrections note above]` markers flag each affected entry; the document is preserved verbatim below per the closed-doc convention.
>
> **kiroku-store closures (in `/Users/shinzui/Keikaku/bokuno/kiroku-project/kiroku/kiroku-store/`):**
>
> 1. **§4.2 Streamly-native single-stream forward read — CLOSED.** Shipped as `Kiroku.Store.Read.readStreamForwardStream :: StreamName -> StreamVersion -> Int32 -> Stream (Eff es) RecordedEvent` (`src/Kiroku/Store/Read.hs:59–75`), re-exported from `Kiroku.Store`. Internally paginates via `Stream.unfoldrM`, sharing SQL path and error semantics with the `Vector`-returning `readStreamForward`. Recorded under "Added — streaming single-stream forward read" in `kiroku-store/CHANGELOG.md`. EP-1's hydration wrapper (`docs/research/06-command-cycle-design.md` §5's `hydrationStream`) can now be retired in favour of the upstream primitive.
> 2. **§4.6 `enrichEvent`/encoder hook in the interpreter — CLOSED.** Shipped as `Kiroku.Store.Settings.StoreSettings { enrichEvent :: Maybe (EventData -> IO EventData), decodeHook :: Maybe (RecordedEvent -> IO RecordedEvent) }` (`src/Kiroku/Store/Settings.hs`), wired through `ConnectionSettings.storeSettings` and copied onto the runtime handle by `withStore`. `enrichEvent` runs on the append path before encoding; `decodeHook` runs on read and subscription paths after decoding. Both default to `Nothing` (`pure` fast path). New hook-aware variants `runTransactionAppendingResource` / `runTransactionAppendingResourceNoRetry` and the `enrichEventsIO :: KirokuStore -> [EventData] -> IO [EventData]` public convenience let direct callers of `appendToStreamTx` apply the hook. Recorded under "Added — interpreter-level event-data hooks (plan 13)" in `kiroku-store/CHANGELOG.md`. EP-1's observability injection (`docs/research/06-command-cycle-design.md` §12) can now wire `keiro.span.trace_id` into `enrichEvent` once and apply uniformly across every kiroku call site.
> 3. **§4.7 `correlation_id`/`causation_id` chain-walking helpers — CLOSED.** Shipped as `Kiroku.Store.Causation` (`src/Kiroku/Store/Causation.hs`, re-exported from `Kiroku.Store`): `findCausationDescendants :: EventId -> Eff es (Vector RecordedEvent)`, `findCausationAncestors :: EventId -> Eff es (Vector RecordedEvent)`, `findByCorrelation :: UUID -> Eff es (Vector RecordedEvent)`. Backed by the existing partial indexes `ix_events_causation_id` and `ix_events_correlation_id` (no schema change). `Kiroku.Store.Effect.Store` gains a single new constructor `FindEvents :: EventFilter -> Store m (Vector RecordedEvent)` whose argument is the closed sum `Kiroku.Store.Types.EventFilter` (`FilterCorrelation` / `FilterCausationDescendants` / `FilterCausationAncestors`) preserving exhaustiveness for mock interpreters. Honours `StoreSettings.decodeHook`. Recorded under "Added — causation chain and correlation walkers (plan 14)" in `kiroku-store/CHANGELOG.md`. The OpenTelemetry-aware helpers (`extractTraceContext`/`injectTraceContext`) noted in §4.7 remain at the application layer.
> 4. **§4.9 `lookupStreamId :: StreamName -> Eff es (Maybe StreamId)` — CLOSED.** Shipped as `Kiroku.Store.Read.lookupStreamId` (`src/Kiroku/Store/Read.hs:163–167`), exported from `Kiroku.Store` and surfaced on the effect via `Kiroku.Store.Effect.LookupStreamId`. Lighter than `getStream` (one `int8` column vs five). EP-4's snapshot writes (`docs/research/09-snapshot-strategy.md` §15) can now resolve name → id without decoding the full `StreamInfo`.
>
> **keiki closures (in `/Users/shinzui/Keikaku/bokuno/keiki/` and the sibling package `keiki-codec-json`):**
>
> 5. **§7.1 register-file `<-> Aeson.Value` helper — CLOSED.** Shipped as the new sibling package `keiki-codec-json` (v0.1.0.0) at `/Users/shinzui/Keikaku/bokuno/keiki/keiki-codec-json/`. Public surface in `keiki-codec-json/src/Keiki/Codec/JSON.hs`: `regFileToJSON :: RegFile rs -> Aeson.Value`, `regFileFromJSON :: Aeson.Value -> Either String (RegFile rs)`, `regFileToEncoding :: RegFile rs -> Aeson.Encoding` (streaming variant for large slot values). TH derivation `deriveRegFileCodec :: Name -> Q [Dec]` and `deriveRegFileCodecAs :: String -> Name -> Q [Dec]` (`keiki-codec-json/src/Keiki/Codec/JSON/TH.hs`) emit per-record `<name>ToJSON`/`<name>ToEncoding`/`<name>FromJSON` from a `Generic` derivation, reusing `Keiki.Generics.RegFieldsOf`/`gToRegFile`/`gFromRegFile`. A separate sibling test-utility package `keiki-codec-json-test` ships `Keiki.Codec.JSON.Test` (round-trip + sensitivity property helpers) and `Keiki.Codec.JSON.Test.Golden` (per-slot-type golden-byte detector — catches the silent `ToJSON` instance change scenario from EP-36 §4 case #10). Implementation tracked by keiki MasterPlan 11 (EPs 36–39); EP-36 (codec + shape hash), EP-38 (TH derivation), EP-39 (property-test toolkit) are Complete; EP-37 (Hackage upload) was *In Progress* at 2026-05-14. The pre-Hackage integration path is keiro's own EP-9 (`docs/plans/9-integrate-keiki-codec-json-into-keiro-snapshot-path.md`) — queued waiting on EP-37.
> 6. **§7.2 register-file shape hash — CLOSED.** Shipped in **core keiki** as `Keiki.Shape` (`/Users/shinzui/Keikaku/bokuno/keiki/src/Keiki/Shape.hs`): `class CanonicalTypeName a` (escape hatch for custom canonical names), `class KnownRegFileShape (rs :: [Slot])`, `regFileShapeHash :: forall rs. KnownRegFileShape rs => Proxy rs -> Text` (SHA-256 hex over canonical `[(Symbol, TypeRep)]` rendering), `regFileShapeCanonical`, `renderStableTypeRep :: SomeTypeRep -> Text` (GHC-upgrade-safe), `sha256Hex`. Hash is byte-equal across GHC versions, cabal rebuilds, and dependency-tree changes; sensitive to slot rename/add/remove/reorder/type change; insensitive to type-class instance changes. Recorded under "`Keiki.Shape` — GHC-upgrade-safe shape hash for snapshot discrimination" in `keiki/CHANGELOG.md` v0.1.0.0; 11 `Keiki.ShapeSpec` golden assertions in the test suite. Reusable by any codec; the JSON-specific helpers live in the sibling `keiki-codec-json`.
>
> **Cascade.** §3's *Block 2 — Parallel work with keiro v1 implementation* loses four of its six items (§§4.2, 5.1 retain — 5.1 is the shibuya-kiroku-adapter `HandlerInTransaction` shape that remains open; §§7.1 and 7.2 close together). Wait — re-reading: §4.2 itself is now CLOSED, so Block 2 retains §4.3 (PG18 docs), §5.1 (`HandlerInTransaction`), and §7.3 (structured error model on `step`/`omega`). Block 4 *Optional* loses §§4.6, 4.7, 4.9 — three of its eight items. Net Block-2 reduction: from six items to three. Net Block-4 reduction: from eight items to five (§§4.8, 4.10, 7.7, 7.8, plus three in shibuya-core/cross-cutting). The implementation MasterPlan that follows the research foundation now inherits the lightest gating posture this document has ever recorded: zero Blocking items, three Wanted Block-2 items, and a smaller Optional tail.
>
> **Still-open keiki items.** §7.3 (structured error model on `step`/`omega`), §7.4 (compile-time inverse-recoverability check), §7.5 (compensate direction on `SymTransducer`), §7.6 (constrained effectful reads in `decide`), §7.7 (Given/When/Then property-test helpers), §7.8 (in-keiki upcaster framework). The keiki CHANGELOG explicitly marks "no built-in serialization" as out of scope (codec lives in sibling packages); §7.8 is therefore unlikely to ever close, which the existing §7.8 *Optional* + §10.1 *Open question* framing already accommodates.
>
> **Still-open kiroku/shibuya items.** §4.3 (PG18 deployment docs), §4.4 (prefix-style category subscription), §4.5 (migration tooling), §4.8 (combined snapshot+tail read), §4.10 (`readStreamUntil`), §5.1 (shibuya-kiroku-adapter `HandlerInTransaction` — the only Wanted-Blocking-for-exactly-once item still open; verified at `/Users/shinzui/Keikaku/bokuno/kiroku-project/kiroku/shibuya-kiroku-adapter/CHANGELOG.md` which records only the 0.1.0.0 initial release with no transactional handler shape), §6.1 (shibuya-core supervised non-adapter worker entry point), §8.3 (`hs-opentelemetry` version coordination — status unchanged).
>
> The parent MasterPlan's 2026-05-14 Surprises & Discoveries entries (one for the kiroku-store closures, one for the keiki-codec-json package shipping) record the cascade. EP-9 (`docs/plans/9-integrate-keiki-codec-json-into-keiro-snapshot-path.md`) is the keiro-side integration plan for §§7.1 + 7.2 — queued waiting on the EP-37 Hackage upload.
...
**Block 1 — Hard prerequisites for keiro v1 implementation.** One item. **[CLOSED 2026-05-10 — see corrections note above; this block is now empty and the implementation MasterPlan is not gated on any upstream item.]**

- *kiroku-store: single-stream `runInTransaction` combinator* (§4.1). **[CLOSED — shipped upstream as `Kiroku.Store.Transaction.runTransactionAppending`; see corrections note above.]**

**Block 2 — Parallel work with keiro v1 implementation, retiring v1 workarounds.** Six items, all Wanted; safe to land in any order. **[After 2026-05-14: only three items remain open — §4.3, §5.1, §7.3. See corrections note above.]**

- *kiroku-store: Streamly-native single-stream forward read* (§4.2). **[CLOSED 2026-05-14 — shipped as `Kiroku.Store.Read.readStreamForwardStream`; see corrections note above.]**
...
- *keiki: register-file `<-> Aeson.Value` helper* (§7.1) — shared with EP-1, EP-2, EP-4. **[CLOSED 2026-05-14 — shipped as `keiki-codec-json` package (`Keiki.Codec.JSON.regFileToJSON`/`regFileFromJSON`); see corrections note above. Keiro-side integration tracked by EP-9.]**
- *keiki: register-file shape hash* (§7.2) — lands alongside §7.1. **[CLOSED 2026-05-14 — shipped in core keiki as `Keiki.Shape.regFileShapeHash`; see corrections note above.]**
...
**Block 4 — Optional, schedule when convenient.** Six items. **[After 2026-05-14: only four items remain open — §§4.8, 4.10, 7.7, 7.8. See corrections note above.]**

- *kiroku-store: `enrichEvent`/encoder hook in the interpreter* (§4.6). **[CLOSED 2026-05-14 — shipped as `Kiroku.Store.Settings.StoreSettings { enrichEvent, decodeHook }`; see corrections note above.]**
- *kiroku-store: `correlation_id`/`causation_id` chain-walking helpers* (§4.7). **[CLOSED 2026-05-14 — shipped as `Kiroku.Store.Causation`; see corrections note above.]**
- *kiroku-store: combined snapshot + tail-events read query* (§4.8).
- *kiroku-store: `lookupStreamId :: StreamName -> Eff es (Maybe StreamId)` helper* (§4.9). **[CLOSED 2026-05-14 — shipped as `Kiroku.Store.Read.lookupStreamId`; see corrections note above.]**
...
### 4.1 Single-stream `runInTransaction` combinator (Blocking) **[CLOSED 2026-05-10 — shipped upstream as `Kiroku.Store.Transaction.runTransactionAppending`; see corrections note at the top of this file. The remainder of this section is preserved verbatim for traceability.]**
...
### 4.2 Streamly-native single-stream forward read (Wanted) **[CLOSED 2026-05-14 — shipped upstream as `Kiroku.Store.Read.readStreamForwardStream`; see corrections note at the top of this file. The remainder of this section is preserved verbatim for traceability.]**
...
### 4.6 `enrichEvent` / encoder hook in the interpreter (Optional) **[CLOSED 2026-05-14 — shipped upstream as `Kiroku.Store.Settings.StoreSettings { enrichEvent, decodeHook }` plus `runTransactionAppendingResource(NoRetry)` and `enrichEventsIO`; see corrections note at the top of this file.]**
...
### 4.7 `correlation_id` / `causation_id` chain-walking helpers (Optional) **[CLOSED 2026-05-14 — shipped upstream as `Kiroku.Store.Causation` (`findCausationDescendants` / `findCausationAncestors` / `findByCorrelation`); see corrections note at the top of this file. The OpenTelemetry-aware `extractTraceContext`/`injectTraceContext` helpers remain at the application layer (kiroku-store does not pull in `hs-opentelemetry`).]**
...
### 4.9 `lookupStreamId :: StreamName -> Eff es (Maybe StreamId)` helper (Optional) **[CLOSED 2026-05-14 — shipped upstream as `Kiroku.Store.Read.lookupStreamId` and on the effect via `Kiroku.Store.Effect.LookupStreamId`; see corrections note at the top of this file.]**
...
### 7.1 Register-file `<-> Aeson.Value` helper (Wanted, Blocking for EP-4) **[CLOSED 2026-05-14 — shipped upstream as the new sibling package `keiki-codec-json` (v0.1.0.0): `Keiki.Codec.JSON.regFileToJSON` / `regFileFromJSON` / `regFileToEncoding`, plus TH derivation `deriveRegFileCodec` / `deriveRegFileCodecAs` and a `keiki-codec-json-test` property/golden toolkit. See corrections note at the top of this file. The keiro-side integration plan is EP-9 (`docs/plans/9-integrate-keiki-codec-json-into-keiro-snapshot-path.md`), queued waiting on EP-37 Hackage upload.]**
...
### 7.2 Register-file shape hash (Wanted, Blocking for EP-4) **[CLOSED 2026-05-14 — shipped upstream in core keiki as `Keiki.Shape.regFileShapeHash` (`KnownRegFileShape rs => Proxy rs -> Text`, SHA-256 over canonical `[(Symbol, TypeRep)]` rendering via `renderStableTypeRep`); see corrections note at the top of this file. Lands together with §7.1 as recommended; the JSON-specific helpers live in the sibling `keiki-codec-json` while the shape hash itself is codec-agnostic and ships in core keiki.]**
...
The keiro research foundation produces six design documents (EP-1 through EP-6) and three working spikes (EP-1, EP-2; EP-3 deferred). The synthesis here consolidates seventeen upstream feature requests across kiroku-store, shibuya-kiroku-adapter, shibuya-core, and keiki, plus three cross-cutting items, plus six explicitly-not-gaps. One item is **Blocking** for keiro v1 implementation: kiroku-store's single-stream `runInTransaction` combinator (§4.1). **[CLOSED 2026-05-10 — see corrections note at the top of this file. The §4.1 item shipped upstream as `Kiroku.Store.Transaction.runTransactionAppending`; Block 1 is now empty.]** Six items are **Wanted** in Block 2 (parallel with v1 implementation): kiroku-store Streamly-native single-stream forward read **[CLOSED 2026-05-14]**, kiroku-store Postgres 18 docs, shibuya-kiroku-adapter `HandlerInTransaction` shape, keiki register-file `<-> Aeson.Value` helper **[CLOSED 2026-05-14]** plus shape hash **[CLOSED 2026-05-14]**, and keiki structured error model on `step`/`omega`. Six items are **Wanted** in Block 3 (v1.x quality-of-life). Eight items are **Optional** in Block 4 — three of which (§4.6 enrichEvent, §4.7 causation walkers, §4.9 lookupStreamId) **also closed 2026-05-14**. Three open questions are forwarded to the upstream maintainers for input.

The keiro implementation MasterPlan that follows this research foundation can begin work as soon as §4.1 lands. **[CLOSED 2026-05-10 — §4.1 has shipped; the implementation MasterPlan can begin work without further upstream gating.]** Every other Block-2 item can land in parallel with the implementation; every Block-3 and Block-4 item retires a workaround the implementation MasterPlan documents. No Blocking item depends on any other Blocking item — the dependency DAG is shallow.
