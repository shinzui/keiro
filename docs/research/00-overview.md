...
`13-agent-qualification-runtime-wiring.md`,
`14-structural-consumer-type-tradeoffs.md`, and
`opentelemetry-semconv-audit.md` sit outside this map: they are standalone notes
written after the research programme closed, not rungs in the original reading
order.
...
> **Current-contract annotation (2026-08-12):** The item 12 summary above describes the
> historical research taxonomy. The current runtime uses `QueryFreshness` with `Immediate`,
> `WaitForHead`, and `WaitForPosition`, and candidate Language 5 separates owner `delivery`
> from query `freshness`. The research document itself carries the detailed correction.

- `14-structural-consumer-type-tradeoffs.md` — Design exploration of the convenience, expressiveness, atomicity, migration, and performance costs imposed by IR-1's structural/opaque consumer-type boundary. Each cost is paired with unsafe examples, the guarantee being protected, and improvement paths that preserve wire authority, structured replay, usage-aware compatibility, symbolic honesty, and private/public ownership.
