...
> **Corrections note (2026-05-14) — multiple §"Gaps for Keiro" items closed upstream.** Four of the ten kiroku-side gaps enumerated below have shipped upstream since this survey was written, retiring the workarounds keiro v1 documented:
>
> - **Gap #7 "No serialization/encryption hooks" — CLOSED.** Shipped as `Kiroku.Store.Settings.StoreSettings { enrichEvent :: Maybe (EventData -> IO EventData), decodeHook :: Maybe (RecordedEvent -> IO RecordedEvent) }` (`kiroku-store/src/Kiroku/Store/Settings.hs`), wired through `ConnectionSettings.storeSettings`. `enrichEvent` runs on the append path before encoding; `decodeHook` runs on read and subscription paths after decoding. Both default to `Nothing` (no-op fast path). Hook-aware variants `runTransactionAppendingResource` / `runTransactionAppendingResourceNoRetry` and the `enrichEventsIO :: KirokuStore -> [EventData] -> IO [EventData]` public convenience let direct callers of `appendToStreamTx` apply the hook. Recorded under "Added — interpreter-level event-data hooks (plan 13)" in `kiroku-store/CHANGELOG.md`.
> - **Gap #8 "No correlation/causation helpers" — CLOSED (chain-walking part).** Shipped as `Kiroku.Store.Causation` (`kiroku-store/src/Kiroku/Store/Causation.hs`, re-exported from `Kiroku.Store`): `findCausationDescendants :: EventId -> Eff es (Vector RecordedEvent)`, `findCausationAncestors :: EventId -> Eff es (Vector RecordedEvent)`, `findByCorrelation :: UUID -> Eff es (Vector RecordedEvent)`. Backed by the existing partial indexes `ix_events_causation_id` / `ix_events_correlation_id` (no schema change). The OpenTelemetry-aware `extractTraceContext` / `injectTraceContext` part of the original gap remains at the application layer (kiroku-store does not pull in `hs-opentelemetry`); the application can build it on top of `enrichEvent` (Gap #7).
> - **Gap #10 "No first-class transaction primitive that wraps `read → append`" — CLOSED.** See the 2026-05-10 corrections note above for the full surface (`Kiroku.Store.Transaction.runTransactionAppending` and siblings).
> - **Bonus closure (related to Gap #4 "No read-decide-append combinator"): `lookupStreamId :: StreamName -> Eff es (Maybe StreamId)`** (`kiroku-store/src/Kiroku/Store/Read.hs:163–167`, surfaced on the effect via `Kiroku.Store.Effect.LookupStreamId`) — lighter than `getStream` (one `int8` column vs five). Useful for keiro's snapshot writes and read-side joins that just need the surrogate id.
> - **Bonus closure: `Kiroku.Store.Read.readStreamForwardStream :: StreamName -> StreamVersion -> Int32 -> Stream (Eff es) RecordedEvent`** (`kiroku-store/src/Kiroku/Store/Read.hs:59–75`) — Streamly-native sibling of `readStreamForward` for constant-memory folds over long streams. Used by EP-1's hydration pipeline.
>
> **Still open at the kiroku layer:** Gap #1 (typed payload codecs — not in kiroku's scope, lives in keiro's EP-2 codec layer plus the new sibling package `keiki-codec-json`), Gap #3 (snapshot read/write — keiro-owned `keiro_snapshots` table per EP-4), Gap #4 (read-decide-append combinator — keiro-owned `runCommand` per EP-1, now built on `runTransactionAppending`), Gap #5 (multi-stream hydration — keiro-owned), Gap #6 (projection-rebuild helper — keiro-owned per EP-8), Gap #9 (point-in-time replay — `readStreamUntil`, still open per `docs/research/11-upstream-roadmap.md` §4.10). The parent MasterPlan's 2026-05-14 Surprises & Discoveries entry "cross-cutting, second wave of kiroku closures" records the cascade. This survey is preserved verbatim below for traceability; inline `[CLOSED]` markers flag each affected line.
...
7. **No serialization/encryption hooks** — no `enrichEvent` interpreter hook. Out-of-band only. **[CLOSED 2026-05-14 — `Kiroku.Store.Settings.StoreSettings { enrichEvent, decodeHook }` shipped; see 2026-05-14 corrections note above.]**
8. **No correlation/causation helpers** — fields are stored, but no tracing-context injection or chain-walking helpers. **[PARTIALLY CLOSED 2026-05-14 — chain-walking shipped as `Kiroku.Store.Causation.findCausationDescendants`/`findCausationAncestors`/`findByCorrelation`; tracing-context injection remains application-layer (now buildable on top of the new `enrichEvent` hook from Gap #7). See 2026-05-14 corrections note above.]**
9. **No point-in-time replay** — no API to read events up to a timestamp/global position.
10. **No first-class transaction primitive** that wraps `read → append` for single streams (only `appendMultiStream` opens a tx). **[CLOSED 2026-05-10 — `Kiroku.Store.Transaction.runTransactionAppending` shipped; see 2026-05-10 corrections note above.]**
