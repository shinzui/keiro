...
behaviorRequirements =
  [ -- IncidentOpen x EscalateIncident: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-0aad69728dee8fec"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = IncidentOpen
      , commandName = "EscalateIncident"
      , expectedEdge = (Just (K.EdgeRef IncidentOpen 1))
      , target = Just IncidentOpen
      , eventKinds = ["IncidentEscalated"]
      }
  , -- IncidentOpen x AcknowledgeIncident: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-36c541281e30554e"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = IncidentOpen
      , commandName = "AcknowledgeIncident"
      , expectedEdge = (Just (K.EdgeRef IncidentOpen 0))
      , target = Just IncidentOpen
      , eventKinds = ["IncidentAcknowledged"]
      }
  ]
...
commandKind command = case command of
  AcknowledgeIncident _ -> "AcknowledgeIncident"
