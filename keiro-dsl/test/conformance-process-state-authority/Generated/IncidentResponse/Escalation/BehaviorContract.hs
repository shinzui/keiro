-- @generated by keiro-dsl 0.19.0.0 (language keiro-dsl 6) from aggregate Escalation; do not edit.
...
data BehaviorRequirement = BehaviorRequirement
  ...
  , source :: !EscalationVertex
  , commandName :: !Text
  , expectedEdge :: !(Maybe (K.EdgeRef EscalationVertex))
  , target :: !(Maybe EscalationVertex)
  , eventKinds :: ![Text]
  }
...
data BehaviorWitness
  ...
  | ReplayWitness
      { key :: BehaviorKey
      , historyPrefix :: [EscalationEvent]
      , observedChunk :: [EscalationEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  ...
  , -- EscalationOpen x NoteRaised: live transition
    BehaviorRequirement
      ...
      , expectedEdge = (Just (K.EdgeRef EscalationOpen 0))
      , target = Just EscalationOpen
...
renderBehaviorConformanceText report = T.unlines
  [ "behavior conformance: Escalation"
...
runLive :: BehaviorRequirement -> [EscalationEvent] -> EscalationCommand -> LiveExpectation -> Either BehaviorFailure ()
...
runAcceptance requirement seed command expectation = case expectation of
  ...
  Emits expectedEvents -> case K.stepDetailedEither escalationTransducer seed command of
    ...
    Right success -> do
      ...
      replayed <- case K.applyEventsDetailedEither escalationTransducer seed decoded of
...
runReplay :: BehaviorRequirement -> [EscalationEvent] -> [EscalationEvent] -> Either BehaviorFailure ()
runReplay requirement prefix chunk = case requirement.kind of
  ReplayTransition -> do
    ...
    replayed <- case K.applyEventsDetailedEither escalationTransducer (K.replaySuccessState settled, K.replaySuccessRegs settled) decoded of
...
checkSingleAttribution :: BehaviorRequirement -> K.EdgeMode -> Int -> [K.ReplayAttribution EscalationVertex] -> Either BehaviorFailure ()
...
settleHistory :: BehaviorRequirement -> Text -> [EscalationEvent] -> Either BehaviorFailure (K.ReplaySuccess EscalationRegs EscalationVertex)
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither escalationTransducer (EscalationOpen, initialEscalationRegs) decoded of
    Left replayFailure -> failure requirement (label <> "-replay-failed") (tshow replayFailure)
    Right replaySuccess -> Right replaySuccess

decodeEvents :: [EscalationEvent] -> Either Text [EscalationEvent]
decodeEvents = traverse (\event -> parseEscalationEvent (Codec.eventType escalationCodec event) (encodeEscalationEvent event))

commandKind :: EscalationCommand -> Text
...
regsEqual :: K.RegFile EscalationRegs -> K.RegFile EscalationRegs -> Bool
regsEqual _ _ = True
