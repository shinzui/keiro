...
data BehaviorRequirement = BehaviorRequirement
  ...
  , source :: !OrderBookVertex
  , commandName :: !Text
  , expectedEdge :: !(Maybe (K.EdgeRef OrderBookVertex))
  , target :: !(Maybe OrderBookVertex)
  , eventKinds :: ![Text]
  }
...
data BehaviorWitness
  ...
  | LiveWitness
      { key :: BehaviorKey
      , history :: [OrderBookEvent]
      , command :: OrderBookCommand
      , expected :: LiveExpectation
      }
  | ReplayWitness
      { key :: BehaviorKey
      , historyPrefix :: [OrderBookEvent]
      , observedChunk :: [OrderBookEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  ...
  , -- OrderBookEmpty x Record: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-a396f60ddb8f99be"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = OrderBookEmpty
      , commandName = "Record"
      , expectedEdge = (Just (K.EdgeRef OrderBookEmpty 0))
      , target = Just OrderBookRecorded
      , eventKinds = ["OrderRecorded"]
      }
  ]
...
runRejection :: BehaviorRequirement -> (OrderBookVertex, K.RegFile OrderBookRegs) -> OrderBookCommand -> LiveExpectation -> Either BehaviorFailure ()
...
runAcceptance :: BehaviorRequirement -> (OrderBookVertex, K.RegFile OrderBookRegs) -> OrderBookCommand -> LiveExpectation -> Either BehaviorFailure ()
...
checkAcceptedEnvelope :: BehaviorRequirement -> K.StepSuccess OrderBookRegs OrderBookVertex OrderBookEvent -> Either BehaviorFailure ()
...
checkSingleAttribution :: BehaviorRequirement -> K.EdgeMode -> Int -> [K.ReplayAttribution OrderBookVertex] -> Either BehaviorFailure ()
...
settleHistory :: BehaviorRequirement -> Text -> [OrderBookEvent] -> Either BehaviorFailure (K.ReplaySuccess OrderBookRegs OrderBookVertex)
...
commandKind :: OrderBookCommand -> Text
commandKind command = case command of
  Record _ -> "Record"

eventKind :: OrderBookEvent -> Text
