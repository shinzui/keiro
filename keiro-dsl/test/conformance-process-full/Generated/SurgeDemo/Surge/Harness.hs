...
sampleEventSurgeThresholdNoted = (SurgeThresholdNoted (SurgeThresholdNotedData (case parseHospitalId "hosp_01h455vb4pex5vsknk084sn02q" of Right parsed -> parsed; Left _ -> error "generated valid ID sample failed to parse")))

sampleEventSurgeTimerFired :: SurgeEvent
sampleEventSurgeTimerFired = (SurgeTimerFired (SurgeTimerFiredData (case parseHospitalId "hosp_01h455vb4pex5vsknk084sn02q" of Right parsed -> parsed; Left _ -> error "generated valid ID sample failed to parse")))

acceptNoteSurgeThreshold :: Bool
acceptNoteSurgeThreshold =
  case step surgeTransducer (SurgeWatching, initialSurgeRegs) ((NoteSurgeThreshold (NoteSurgeThresholdData (case parseHospitalId "hosp_01h455vb4pex5vsknk084sn02q" of Right parsed -> parsed; Left _ -> error "generated valid ID sample failed to parse")))) of
