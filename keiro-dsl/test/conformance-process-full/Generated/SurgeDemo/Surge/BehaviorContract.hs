...
behaviorRequirements =
  [ -- SurgeFired x MarkSurgeTimerFired: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-364cc8c97a1dccd7"
  ...
  , -- SurgeWatching x NoteSurgeThreshold: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-7bd417bb60f14aea"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = SurgeWatching
      , commandName = "NoteSurgeThreshold"
      , expectedEdge = (Just (K.EdgeRef SurgeWatching 0))
      , target = Just SurgeNoted
      , eventKinds = ["SurgeThresholdNoted"]
      }
  , -- SurgeWatching x MarkSurgeTimerFired: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-8303777877a78d98"
      , kind = RequiredRejection
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = SurgeWatching
  ...
  , -- SurgeFired x NoteSurgeThreshold: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-9d1377f4e60cd45a"
      , kind = RequiredRejection
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = SurgeFired
  ...
  , -- SurgeNoted x NoteSurgeThreshold: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-c7783d8bb46678fe"
      , kind = RequiredRejection
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = SurgeNoted
  ...
  , -- SurgeNoted x MarkSurgeTimerFired: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-db2fec76864e5867"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = SurgeNoted
      , commandName = "MarkSurgeTimerFired"
      , expectedEdge = (Just (K.EdgeRef SurgeNoted 0))
      , target = Just SurgeFired
      , eventKinds = ["SurgeTimerFired"]
      }
  ]
...
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither surgeTransducer (SurgeWatching, initialSurgeRegs) decoded of
