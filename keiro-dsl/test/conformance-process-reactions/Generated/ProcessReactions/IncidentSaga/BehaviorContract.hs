...
import Generated.ProcessReactions.BehaviorSourceMap qualified as BehaviorSourceMap
...
behaviorRequirements =
  [ -- IncidentSagaOpen x RecordCritical: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-6b66722f9f2463d3"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = IncidentSagaOpen
      , commandName = "RecordCritical"
      , expectedEdge = (Just (K.EdgeRef IncidentSagaOpen 0))
      , target = Just IncidentSagaOpen
      , eventKinds = ["CriticalRecorded"]
      }
  , -- IncidentSagaOpen x RecordRoutine: live transition
    BehaviorRequirement
      ...
      , guardCoverage = GuardTotal
      , source = IncidentSagaOpen
      , commandName = "RecordRoutine"
      , expectedEdge = (Just (K.EdgeRef IncidentSagaOpen 1))
      , target = Just IncidentSagaOpen
      , eventKinds = ["RoutineRecorded"]
      }
  ]
...
commandKind command = case command of
  RecordCritical _ -> "RecordCritical"
  RecordRoutine _ -> "RecordRoutine"
