...
module Generated.ProcessReactionsMinimal.Incident.BehaviorContract
  ( BehaviorKey (..)
  , unBehaviorKey
  , ObligationKind (..)
  , EvidenceLevel (..)
...
import Generated.ProcessReactionsMinimal.Incident.Codec (encodeIncidentEvent, parseIncidentEvent, incidentCodec)
import Generated.ProcessReactionsMinimal.Incident.Domain
import Generated.ProcessReactionsMinimal.Incident.Transducer (incidentTransducer)
...
data BehaviorRequirement = BehaviorRequirement
  ...
  , source :: !IncidentVertex
  , commandName :: !Text
  , expectedEdge :: !(Maybe (K.EdgeRef IncidentVertex))
  , target :: !(Maybe IncidentVertex)
  , eventKinds :: ![Text]
  }
...
data BehaviorWitness
  ...
  | ReplayWitness
      { key :: BehaviorKey
      , historyPrefix :: [IncidentEvent]
      , observedChunk :: [IncidentEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  [ -- IncidentOpen x EscalateIncident: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-b80bea9f8d7d40e8"
      , kind = LiveTransition
...
runLive :: BehaviorRequirement -> [IncidentEvent] -> IncidentCommand -> LiveExpectation -> Either BehaviorFailure ()
...
runRejection :: BehaviorRequirement -> (IncidentVertex, K.RegFile IncidentRegs) -> IncidentCommand -> LiveExpectation -> Either BehaviorFailure ()
runRejection requirement seed command expectation = case expectation of
  ...
  Rejects expectedClass -> case K.stepDetailedEither incidentTransducer seed command of
...
runAcceptance :: BehaviorRequirement -> (IncidentVertex, K.RegFile IncidentRegs) -> IncidentCommand -> LiveExpectation -> Either BehaviorFailure ()
runAcceptance requirement seed command expectation = case expectation of
  ...
  NoOp -> case K.stepDetailedEither incidentTransducer seed command of
  ...
  Emits expectedEvents -> case K.stepDetailedEither incidentTransducer seed command of
    Left stepFailure -> failure requirement "unexpected-rejection" (tshow stepFailure)
    Right success -> do
      checkAcceptedEnvelope requirement success
      ...
      replayed <- case K.applyEventsDetailedEither incidentTransducer seed decoded of
...
checkAcceptedEnvelope :: BehaviorRequirement -> K.StepSuccess IncidentRegs IncidentVertex IncidentEvent -> Either BehaviorFailure ()
...
runReplay :: BehaviorRequirement -> [IncidentEvent] -> [IncidentEvent] -> Either BehaviorFailure ()
runReplay requirement prefix chunk = case requirement.kind of
  ReplayTransition -> do
    ...
    replayed <- case K.applyEventsDetailedEither incidentTransducer (K.replaySuccessState settled, K.replaySuccessRegs settled) decoded of
...
checkSingleAttribution :: BehaviorRequirement -> K.EdgeMode -> Int -> [K.ReplayAttribution IncidentVertex] -> Either BehaviorFailure ()
...
settleHistory :: BehaviorRequirement -> Text -> [IncidentEvent] -> Either BehaviorFailure (K.ReplaySuccess IncidentRegs IncidentVertex)
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither incidentTransducer (IncidentOpen, initialIncidentRegs) decoded of
...
decodeEvents = traverse (\event -> parseIncidentEvent (Codec.eventType incidentCodec event) (encodeIncidentEvent event))

commandKind :: IncidentCommand -> Text
commandKind command = case command of
  EscalateIncident _ -> "EscalateIncident"

eventKind :: IncidentEvent -> Text
eventKind event = case Codec.eventType incidentCodec event of Codec.EventType tag -> tag

regsEqual :: K.RegFile IncidentRegs -> K.RegFile IncidentRegs -> Bool
regsEqual _ _ = True
