...
module Generated.ProcessReactionsMinimal.IncidentSaga.BehaviorContract
  ( BehaviorKey (..)
  , unBehaviorKey
  , ObligationKind (..)
  , EvidenceLevel (..)
...
import Generated.ProcessReactionsMinimal.IncidentSaga.Codec (encodeIncidentSagaEvent, parseIncidentSagaEvent, incidentSagaCodec)
import Generated.ProcessReactionsMinimal.IncidentSaga.Domain
import Generated.ProcessReactionsMinimal.IncidentSaga.Transducer (incidentSagaTransducer)
import Generated.ProcessReactionsMinimal.BehaviorSourceMap qualified as BehaviorSourceMap
...
data BehaviorRequirement = BehaviorRequirement
  ...
  , source :: !IncidentSagaVertex
  , commandName :: !Text
  , expectedEdge :: !(Maybe (K.EdgeRef IncidentSagaVertex))
  , target :: !(Maybe IncidentSagaVertex)
  , eventKinds :: ![Text]
  }
...
data LiveExpectation
  = Emits (NonEmpty IncidentSagaEvent)
...
data BehaviorWitness
  ...
  | LiveWitness
      { key :: BehaviorKey
      , history :: [IncidentSagaEvent]
      , command :: IncidentSagaCommand
      , expected :: LiveExpectation
      }
  | ReplayWitness
      { key :: BehaviorKey
      , historyPrefix :: [IncidentSagaEvent]
      , observedChunk :: [IncidentSagaEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  [ -- IncidentSagaOpen x RecordIncident: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-d8d2518bf8eda97e"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = IncidentSagaOpen
      , commandName = "RecordIncident"
      , expectedEdge = (Just (K.EdgeRef IncidentSagaOpen 0))
      , target = Just IncidentSagaOpen
      , eventKinds = ["IncidentRecorded"]
      }
  ]
...
renderBehaviorConformanceText report = T.unlines
  [ "behavior conformance: IncidentSaga"
...
runLive :: BehaviorRequirement -> [IncidentSagaEvent] -> IncidentSagaCommand -> LiveExpectation -> Either BehaviorFailure ()
...
runRejection :: BehaviorRequirement -> (IncidentSagaVertex, K.RegFile IncidentSagaRegs) -> IncidentSagaCommand -> LiveExpectation -> Either BehaviorFailure ()
runRejection requirement seed command expectation = case expectation of
  ...
  Rejects expectedClass -> case K.stepDetailedEither incidentSagaTransducer seed command of
...
runAcceptance :: BehaviorRequirement -> (IncidentSagaVertex, K.RegFile IncidentSagaRegs) -> IncidentSagaCommand -> LiveExpectation -> Either BehaviorFailure ()
runAcceptance requirement seed command expectation = case expectation of
  ...
  NoOp -> case K.stepDetailedEither incidentSagaTransducer seed command of
  ...
  Emits expectedEvents -> case K.stepDetailedEither incidentSagaTransducer seed command of
    Left stepFailure -> failure requirement "unexpected-rejection" (tshow stepFailure)
    Right success -> do
      checkAcceptedEnvelope requirement success
      ...
      replayed <- case K.applyEventsDetailedEither incidentSagaTransducer seed decoded of
...
checkAcceptedEnvelope :: BehaviorRequirement -> K.StepSuccess IncidentSagaRegs IncidentSagaVertex IncidentSagaEvent -> Either BehaviorFailure ()
...
runReplay :: BehaviorRequirement -> [IncidentSagaEvent] -> [IncidentSagaEvent] -> Either BehaviorFailure ()
runReplay requirement prefix chunk = case requirement.kind of
  ReplayTransition -> do
    ...
    replayed <- case K.applyEventsDetailedEither incidentSagaTransducer (K.replaySuccessState settled, K.replaySuccessRegs settled) decoded of
...
checkSingleAttribution :: BehaviorRequirement -> K.EdgeMode -> Int -> [K.ReplayAttribution IncidentSagaVertex] -> Either BehaviorFailure ()
...
settleHistory :: BehaviorRequirement -> Text -> [IncidentSagaEvent] -> Either BehaviorFailure (K.ReplaySuccess IncidentSagaRegs IncidentSagaVertex)
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither incidentSagaTransducer (IncidentSagaOpen, initialIncidentSagaRegs) decoded of
    Left replayFailure -> failure requirement (label <> "-replay-failed") (tshow replayFailure)
    Right replaySuccess -> Right replaySuccess

decodeEvents :: [IncidentSagaEvent] -> Either Text [IncidentSagaEvent]
decodeEvents = traverse (\event -> parseIncidentSagaEvent (Codec.eventType incidentSagaCodec event) (encodeIncidentSagaEvent event))

commandKind :: IncidentSagaCommand -> Text
commandKind command = case command of
  RecordIncident _ -> "RecordIncident"

eventKind :: IncidentSagaEvent -> Text
eventKind event = case Codec.eventType incidentSagaCodec event of Codec.EventType tag -> tag

regsEqual :: K.RegFile IncidentSagaRegs -> K.RegFile IncidentSagaRegs -> Bool
regsEqual _ _ = True
