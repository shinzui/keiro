...
-- @generated by keiro-dsl 0.15.0.0 (language keiro-dsl 4) from aggregate ScalarLedger; do not edit.
...
data BehaviorRequirement = BehaviorRequirement
  ...
  , source :: !ScalarLedgerVertex
  , commandName :: !Text
  , expectedEdge :: !(Maybe (K.EdgeRef ScalarLedgerVertex))
  , target :: !(Maybe ScalarLedgerVertex)
  , eventKinds :: ![Text]
  }
...
data LiveExpectation
  = Emits (NonEmpty ScalarLedgerEvent)
...
data BehaviorWitness
  ...
  | LiveWitness
      { key :: BehaviorKey
      , history :: [ScalarLedgerEvent]
      , command :: ScalarLedgerCommand
      , expected :: LiveExpectation
      }
  | ReplayWitness
      { key :: BehaviorKey
      , historyPrefix :: [ScalarLedgerEvent]
      , observedChunk :: [ScalarLedgerEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  [ -- ScalarLedgerEmpty x Record: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-4e461a9ba9271054"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardUnknown
      , source = ScalarLedgerEmpty
      , commandName = "Record"
      , expectedEdge = (Just (K.EdgeRef ScalarLedgerEmpty 0))
      , target = Just ScalarLedgerRecorded
      , eventKinds = ["ScalarsRecorded"]
      }
  , -- ScalarLedgerRecorded x Record: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-d15e326edfa1a438"
      , kind = RequiredRejection
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = ScalarLedgerRecorded
      , commandName = "Record"
...
renderBehaviorConformanceText report = T.unlines
  [ "behavior conformance: ScalarLedger"
...
runLive :: BehaviorRequirement -> [ScalarLedgerEvent] -> ScalarLedgerCommand -> LiveExpectation -> Either BehaviorFailure ()
...
runRejection :: BehaviorRequirement -> (ScalarLedgerVertex, K.RegFile ScalarLedgerRegs) -> ScalarLedgerCommand -> LiveExpectation -> Either BehaviorFailure ()
runRejection requirement seed command expectation = case expectation of
  ...
  Rejects expectedClass -> case K.stepDetailedEither scalarLedgerTransducer seed command of
...
runAcceptance :: BehaviorRequirement -> (ScalarLedgerVertex, K.RegFile ScalarLedgerRegs) -> ScalarLedgerCommand -> LiveExpectation -> Either BehaviorFailure ()
runAcceptance requirement seed command expectation = case expectation of
  ...
  NoOp -> case K.stepDetailedEither scalarLedgerTransducer seed command of
  ...
  Emits expectedEvents -> case K.stepDetailedEither scalarLedgerTransducer seed command of
    Left stepFailure -> failure requirement "unexpected-rejection" (tshow stepFailure)
    Right success -> do
      checkAcceptedEnvelope requirement success
      ...
      replayed <- case K.applyEventsDetailedEither scalarLedgerTransducer seed decoded of
...
checkAcceptedEnvelope :: BehaviorRequirement -> K.StepSuccess ScalarLedgerRegs ScalarLedgerVertex ScalarLedgerEvent -> Either BehaviorFailure ()
...
runReplay :: BehaviorRequirement -> [ScalarLedgerEvent] -> [ScalarLedgerEvent] -> Either BehaviorFailure ()
runReplay requirement prefix chunk = case requirement.kind of
  ReplayTransition -> do
    ...
    replayed <- case K.applyEventsDetailedEither scalarLedgerTransducer (K.replaySuccessState settled, K.replaySuccessRegs settled) decoded of
...
checkSingleAttribution :: BehaviorRequirement -> K.EdgeMode -> Int -> [K.ReplayAttribution ScalarLedgerVertex] -> Either BehaviorFailure ()
...
settleHistory :: BehaviorRequirement -> Text -> [ScalarLedgerEvent] -> Either BehaviorFailure (K.ReplaySuccess ScalarLedgerRegs ScalarLedgerVertex)
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither scalarLedgerTransducer (ScalarLedgerEmpty, initialScalarLedgerRegs) decoded of
    Left replayFailure -> failure requirement (label <> "-replay-failed") (tshow replayFailure)
    Right replaySuccess -> Right replaySuccess

decodeEvents :: [ScalarLedgerEvent] -> Either Text [ScalarLedgerEvent]
decodeEvents = traverse (\event -> parseScalarLedgerEvent (Codec.eventType scalarLedgerCodec event) (encodeScalarLedgerEvent event))

commandKind :: ScalarLedgerCommand -> Text
commandKind command = case command of
  Record _ -> "Record"

eventKind :: ScalarLedgerEvent -> Text
eventKind event = case Codec.eventType scalarLedgerCodec event of Codec.EventType tag -> tag

regsEqual :: K.RegFile ScalarLedgerRegs -> K.RegFile ScalarLedgerRegs -> Bool
regsEqual left right = (left K.! #observedAt) == (right K.! #observedAt) && (left K.! #revision) == (right K.! #revision)
