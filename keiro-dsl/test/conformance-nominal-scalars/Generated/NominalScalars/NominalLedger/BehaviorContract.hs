...
import Generated.NominalScalars.BehaviorSourceMap qualified as BehaviorSourceMap
...
data BehaviorRequirement = BehaviorRequirement
  ...
  , source :: !NominalLedgerVertex
  , commandName :: !Text
  , expectedEdge :: !(Maybe (K.EdgeRef NominalLedgerVertex))
  , target :: !(Maybe NominalLedgerVertex)
  , eventKinds :: ![Text]
  }
...
data BehaviorWitness
  ...
  | LiveWitness
      { key :: BehaviorKey
      , history :: [NominalLedgerEvent]
      , command :: NominalLedgerCommand
      , expected :: LiveExpectation
      }
  | ReplayWitness
      { key :: BehaviorKey
      , historyPrefix :: [NominalLedgerEvent]
      , observedChunk :: [NominalLedgerEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  [ -- NominalLedgerEmpty x RecordNominals: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-274eaf83d346048a"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardUnknown
      , source = NominalLedgerEmpty
      , commandName = "RecordNominals"
      , expectedEdge = (Just (K.EdgeRef NominalLedgerEmpty 0))
      , target = Just NominalLedgerRecorded
      , eventKinds = ["NominalsRecorded"]
      }
  , -- NominalLedgerRecorded x RecordNominals: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-971c0c80ffdfe018"
      , kind = RequiredRejection
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = NominalLedgerRecorded
      , commandName = "RecordNominals"
