...
-- @generated by keiro-dsl 0.13.0.0 (language keiro-dsl 5) from aggregate Hospital; do not edit.
...
behaviorRequirements =
  [ -- HospitalRouted x RouteAcceptedTransferNeed: required rejection
    BehaviorRequirement
      { requirementKey = BehaviorKey "behavior-v1-51a443ca8bbb0eab"
      , requirementKind = RequiredRejection
      , requirementEvidence = GeneratedAuthoritative
      , requirementGuardCoverage = GuardNotApplicable
      , requirementSource = HospitalRouted
      , requirementCommandName = "RouteAcceptedTransferNeed"
      , requirementExpectedEdge = Nothing
      , requirementTarget = Nothing
      , requirementEventKinds = []
      }
  , -- HospitalOpen x RouteAcceptedTransferNeed: live transition
    BehaviorRequirement
      { requirementKey = BehaviorKey "behavior-v1-6dc56a81dec7fe2b"
      , requirementKind = LiveTransition
      , requirementEvidence = GeneratedAuthoritative
      , requirementGuardCoverage = GuardTotal
      , requirementSource = HospitalOpen
      , requirementCommandName = "RouteAcceptedTransferNeed"
      , requirementExpectedEdge = (Just (K.EdgeRef HospitalOpen 0))
      , requirementTarget = Just HospitalRouted
      , requirementEventKinds = ["TransferNeedRouted"]
      }
  ]
...
runRejection :: BehaviorRequirement -> (HospitalVertex, K.RegFile HospitalRegs) -> HospitalCommand -> LiveExpectation -> Either BehaviorFailure ()
runRejection requirement seed command expectation = case expectation of
  ...
  Rejects expectedClass -> case K.stepDetailedEither hospitalTransducer seed command of
...
runAcceptance :: BehaviorRequirement -> (HospitalVertex, K.RegFile HospitalRegs) -> HospitalCommand -> LiveExpectation -> Either BehaviorFailure ()
runAcceptance requirement seed command expectation = case expectation of
  ...
  NoOp -> case K.stepDetailedEither hospitalTransducer seed command of
  ...
  Emits expectedEvents -> case K.stepDetailedEither hospitalTransducer seed command of
    Left stepFailure -> failure requirement "unexpected-rejection" (tshow stepFailure)
    Right success -> do
      checkAcceptedEnvelope requirement success
...
settleHistory :: BehaviorRequirement -> Text -> [HospitalEvent] -> Either BehaviorFailure (K.ReplaySuccess HospitalRegs HospitalVertex)
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither hospitalTransducer (HospitalOpen, initialHospitalRegs) decoded of
    Left replayFailure -> failure requirement (label <> "-replay-failed") (tshow replayFailure)
    Right replaySuccess -> Right replaySuccess

decodeEvents :: [HospitalEvent] -> Either Text [HospitalEvent]
decodeEvents = traverse (\event -> parseHospitalEvent (Codec.eventType hospitalCodec event) (encodeHospitalEvent event))

commandKind :: HospitalCommand -> Text
commandKind command = case command of
  RouteAcceptedTransferNeed _ -> "RouteAcceptedTransferNeed"

eventKind :: HospitalEvent -> Text
eventKind event = case Codec.eventType hospitalCodec event of Codec.EventType tag -> tag

regsEqual :: K.RegFile HospitalRegs -> K.RegFile HospitalRegs -> Bool
regsEqual left right = (left K.! #routed) == (right K.! #routed)
