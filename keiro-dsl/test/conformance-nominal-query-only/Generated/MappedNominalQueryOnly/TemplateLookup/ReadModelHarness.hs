...
import Generated.MappedNominalQueryOnly.ProjectionCatalog qualified as ProjectionCatalog
...
readModelFacts =
  [ ("registryName", "mapped-nominal-query-only-template-lookup", T.unpack templateLookupReadModel.name)
...
catalogFactsAgainst registrations asyncRegistrations supplies =
  [ ("catalogRegistration", "mapped-nominal-query-only-template-lookup|1|fnv1a:3c07a19c552c3547|template_rebuild", renderRegistration [entry | entry <- registrations, Catalog.queryModelIdText entry.queryModelId == "template_lookup"])
