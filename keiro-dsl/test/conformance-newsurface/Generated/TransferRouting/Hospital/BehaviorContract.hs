...
behaviorRequirements =
  [ -- HospitalAccepting x RouteAcceptedTransferNeed: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-8c1612ae6464bb11"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = HospitalAccepting
      , commandName = "RouteAcceptedTransferNeed"
      , expectedEdge = (Just (K.EdgeRef HospitalAccepting 0))
      , target = Just HospitalAccepting
      , eventKinds = ["AcceptedTransferNeedRouted"]
      }
  ]
...
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither hospitalTransducer (HospitalAccepting, initialHospitalRegs) decoded of
