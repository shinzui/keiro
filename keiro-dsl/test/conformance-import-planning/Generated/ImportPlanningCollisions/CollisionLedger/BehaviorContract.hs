-- @generated by keiro-dsl 0.16.0.0 (language keiro-dsl 4) from aggregate CollisionLedger; do not edit.
...
data BehaviorRequirement = BehaviorRequirement
  ...
  , source :: !CollisionLedgerVertex
  , commandName :: !Text
  , expectedEdge :: !(Maybe (K.EdgeRef CollisionLedgerVertex))
  , target :: !(Maybe CollisionLedgerVertex)
  , eventKinds :: ![Text]
  }
...
data BehaviorWitness
  ...
  | LiveWitness
      { key :: BehaviorKey
      , history :: [CollisionLedgerEvent]
      , command :: CollisionLedgerCommand
      , expected :: LiveExpectation
      }
  | ReplayWitness
      { key :: BehaviorKey
      , historyPrefix :: [CollisionLedgerEvent]
      , observedChunk :: [CollisionLedgerEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  [ -- CollisionLedgerEmpty x Record: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-2134fce4a19c59d7"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = CollisionLedgerEmpty
      , commandName = "Record"
      , expectedEdge = (Just (K.EdgeRef CollisionLedgerEmpty 0))
      , target = Just CollisionLedgerRecorded
      , eventKinds = ["RecordedValues"]
      }
  , -- CollisionLedgerRecorded x Record: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-995f9bf710ce7c6c"
      , kind = RequiredRejection
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = CollisionLedgerRecorded
      , commandName = "Record"
...
runRejection :: BehaviorRequirement -> (CollisionLedgerVertex, K.RegFile CollisionLedgerRegs) -> CollisionLedgerCommand -> LiveExpectation -> Either BehaviorFailure ()
...
runAcceptance :: BehaviorRequirement -> (CollisionLedgerVertex, K.RegFile CollisionLedgerRegs) -> CollisionLedgerCommand -> LiveExpectation -> Either BehaviorFailure ()
...
checkAcceptedEnvelope :: BehaviorRequirement -> K.StepSuccess CollisionLedgerRegs CollisionLedgerVertex CollisionLedgerEvent -> Either BehaviorFailure ()
...
checkSingleAttribution :: BehaviorRequirement -> K.EdgeMode -> Int -> [K.ReplayAttribution CollisionLedgerVertex] -> Either BehaviorFailure ()
...
settleHistory :: BehaviorRequirement -> Text -> [CollisionLedgerEvent] -> Either BehaviorFailure (K.ReplaySuccess CollisionLedgerRegs CollisionLedgerVertex)
...
commandKind :: CollisionLedgerCommand -> Text
commandKind command = case command of
  Record _ -> "Record"

eventKind :: CollisionLedgerEvent -> Text
