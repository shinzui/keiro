...
expected =
    ...
    , ("onAmbiguous", "Retry") -- definition bugs retry to the timer ceiling
    , ("onFailed", "Retry") -- a real failure retries the source event
    , ("rejectedPolicy", "halt") -- rejection-class failures halt loudly
    , ("poisonPolicy", "halt") -- undecodable messages halt loudly
