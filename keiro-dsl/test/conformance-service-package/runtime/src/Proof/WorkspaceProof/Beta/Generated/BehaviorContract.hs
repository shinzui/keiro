...
data BehaviorRequirement = BehaviorRequirement
  ...
  , source :: !BetaVertex
  , commandName :: !Text
  , expectedEdge :: !(Maybe (K.EdgeRef BetaVertex))
  , target :: !(Maybe BetaVertex)
  , eventKinds :: ![Text]
  }
...
data BehaviorWitness
  ...
  | ReplayWitness
      { key :: BehaviorKey
      , historyPrefix :: [BetaEvent]
      , observedChunk :: [BetaEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  [ -- BetaActive x PingBeta: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-6ab7fee2c8aa4e1d"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = BetaActive
      , commandName = "PingBeta"
      , expectedEdge = (Just (K.EdgeRef BetaActive 0))
      , target = Just BetaActive
      , eventKinds = ["BetaPinged"]
      }
  ]
