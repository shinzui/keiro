...
data BehaviorRequirement = BehaviorRequirement
  ...
  , source :: !AlphaVertex
  , commandName :: !Text
  , expectedEdge :: !(Maybe (K.EdgeRef AlphaVertex))
  , target :: !(Maybe AlphaVertex)
  , eventKinds :: ![Text]
  }
...
data BehaviorWitness
  ...
  | ReplayWitness
      { key :: BehaviorKey
      , historyPrefix :: [AlphaEvent]
      , observedChunk :: [AlphaEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  [ -- AlphaActive x LegacyPingAlpha: replay-only transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-579df4e5e81f7729"
      , kind = ReplayTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = AlphaActive
      , commandName = "LegacyPingAlpha"
      , expectedEdge = (Just (K.EdgeRef AlphaActive 2))
      , target = Just AlphaActive
      , eventKinds = ["LegacyAlphaPinged"]
      }
  , -- AlphaActive x PingAlpha: replay-only transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-72a96c964b44a2b6"
      , kind = ReplayTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = AlphaActive
      , commandName = "PingAlpha"
      , expectedEdge = (Just (K.EdgeRef AlphaActive 1))
      , target = Just AlphaActive
      , eventKinds = ["AlphaPinged"]
      }
  , -- AlphaActive x PingAlpha: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-baf43b4c31db955a"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardUnknown
      , source = AlphaActive
      , commandName = "PingAlpha"
      , expectedEdge = (Just (K.EdgeRef AlphaActive 0))
      , target = Just AlphaActive
      , eventKinds = ["AlphaPinged"]
      }
  , -- AlphaActive x LegacyPingAlpha: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-c062ccf5e9b6f5c4"
...
runLive :: BehaviorRequirement -> [AlphaEvent] -> AlphaCommand -> LiveExpectation -> Either BehaviorFailure ()
...
runRejection :: BehaviorRequirement -> (AlphaVertex, K.RegFile AlphaRegs) -> AlphaCommand -> LiveExpectation -> Either BehaviorFailure ()
runRejection requirement seed command expectation = case expectation of
  ...
  Rejects expectedClass -> case K.stepDetailedEither alphaTransducer seed command of
...
runAcceptance :: BehaviorRequirement -> (AlphaVertex, K.RegFile AlphaRegs) -> AlphaCommand -> LiveExpectation -> Either BehaviorFailure ()
runAcceptance requirement seed command expectation = case expectation of
  ...
  NoOp -> case K.stepDetailedEither alphaTransducer seed command of
  ...
  Emits expectedEvents -> case K.stepDetailedEither alphaTransducer seed command of
    Left stepFailure -> failure requirement "unexpected-rejection" (tshow stepFailure)
    Right success -> do
      checkAcceptedEnvelope requirement success
      ...
      replayed <- case K.applyEventsDetailedEither alphaTransducer seed decoded of
...
checkAcceptedEnvelope :: BehaviorRequirement -> K.StepSuccess AlphaRegs AlphaVertex AlphaEvent -> Either BehaviorFailure ()
...
runReplay :: BehaviorRequirement -> [AlphaEvent] -> [AlphaEvent] -> Either BehaviorFailure ()
runReplay requirement prefix chunk = case requirement.kind of
  ReplayTransition -> do
    ...
    replayed <- case K.applyEventsDetailedEither alphaTransducer (K.replaySuccessState settled, K.replaySuccessRegs settled) decoded of
...
checkSingleAttribution :: BehaviorRequirement -> K.EdgeMode -> Int -> [K.ReplayAttribution AlphaVertex] -> Either BehaviorFailure ()
...
settleHistory :: BehaviorRequirement -> Text -> [AlphaEvent] -> Either BehaviorFailure (K.ReplaySuccess AlphaRegs AlphaVertex)
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither alphaTransducer (AlphaActive, initialAlphaRegs) decoded of
    Left replayFailure -> failure requirement (label <> "-replay-failed") (tshow replayFailure)
    Right replaySuccess -> Right replaySuccess

decodeEvents :: [AlphaEvent] -> Either Text [AlphaEvent]
decodeEvents = traverse (\event -> parseAlphaEvent (Codec.eventType alphaCodec event) (encodeAlphaEvent event))

commandKind :: AlphaCommand -> Text
commandKind command = case command of
  PingAlpha _ -> "PingAlpha"
  LegacyPingAlpha _ -> "LegacyPingAlpha"

eventKind :: AlphaEvent -> Text
eventKind event = case Codec.eventType alphaCodec event of Codec.EventType tag -> tag

regsEqual :: K.RegFile AlphaRegs -> K.RegFile AlphaRegs -> Bool
regsEqual _ _ = True
