...
module SkelAggregate.Generated.MyService.Thing.BehaviorContract
  ( BehaviorKey (..)
  , ObligationKind (..)
  , EvidenceLevel (..)
  , GuardCoverage (..)
...
behaviorRequirements =
  [ -- ThingDone x DoThing: required rejection (spec line 9)
    BehaviorRequirement
      { requirementKey = BehaviorKey "behavior-v1-9879aa7cdadb6004"
      , requirementKind = RequiredRejection
      , requirementEvidence = GeneratedAuthoritative
      , requirementGuardCoverage = GuardNotApplicable
      , requirementSource = ThingDone
      , requirementCommandName = "DoThing"
      , requirementExpectedEdge = Nothing
      , requirementTarget = Nothing
      , requirementEventKinds = []
      , requirementLine = 9
      }
  , -- ThingPending x DoThing: live transition (spec line 14)
    BehaviorRequirement
      { requirementKey = BehaviorKey "behavior-v1-c1b356b00484077a"
      , requirementKind = LiveTransition
      , requirementEvidence = GeneratedAuthoritative
      , requirementGuardCoverage = GuardTotal
      , requirementSource = ThingPending
      , requirementCommandName = "DoThing"
      , requirementExpectedEdge = (Just (K.EdgeRef ThingPending 0))
      , requirementTarget = Just ThingDone
      , requirementEventKinds = ["ThingCompleted"]
      , requirementLine = 14
      }
  ]
...
runRejection :: BehaviorRequirement -> (ThingVertex, K.RegFile ThingRegs) -> ThingCommand -> LiveExpectation -> Either BehaviorFailure ()
...
runAcceptance :: BehaviorRequirement -> (ThingVertex, K.RegFile ThingRegs) -> ThingCommand -> LiveExpectation -> Either BehaviorFailure ()
...
checkAcceptedEnvelope :: BehaviorRequirement -> K.StepSuccess ThingRegs ThingVertex ThingEvent -> Either BehaviorFailure ()
...
checkSingleAttribution :: BehaviorRequirement -> K.EdgeMode -> Int -> [K.ReplayAttribution ThingVertex] -> Either BehaviorFailure ()
...
settleHistory :: BehaviorRequirement -> Text -> [ThingEvent] -> Either BehaviorFailure (K.ReplaySuccess ThingRegs ThingVertex)
...
commandKind :: ThingCommand -> Text
commandKind command = case command of
  DoThing _ -> "DoThing"

eventKind :: ThingEvent -> Text
