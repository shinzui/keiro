...
data BehaviorRequirement = BehaviorRequirement
  ...
  , source :: !ThingVertex
  , commandName :: !Text
  , expectedEdge :: !(Maybe (K.EdgeRef ThingVertex))
  , target :: !(Maybe ThingVertex)
  , eventKinds :: ![Text]
  , line :: !Int
  }
...
data BehaviorWitness
  ...
  | ReplayWitness
      { key :: BehaviorKey
      , historyPrefix :: [ThingEvent]
      , observedChunk :: [ThingEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  [ -- ThingDone x DoThing: required rejection (spec line 9)
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-9879aa7cdadb6004"
  ...
  , -- ThingPending x DoThing: live transition (spec line 14)
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-c1b356b00484077a"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = ThingPending
      , commandName = "DoThing"
      , expectedEdge = (Just (K.EdgeRef ThingPending 0))
      , target = Just ThingDone
      , eventKinds = ["ThingCompleted"]
      , line = 14
      }
  ]
