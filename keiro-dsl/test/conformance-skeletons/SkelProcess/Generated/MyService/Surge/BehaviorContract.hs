...
data BehaviorRequirement = BehaviorRequirement
  ...
  , source :: !SurgeVertex
  , commandName :: !Text
  , expectedEdge :: !(Maybe (K.EdgeRef SurgeVertex))
  , target :: !(Maybe SurgeVertex)
  , eventKinds :: ![Text]
  , line :: !Int
  }
...
data BehaviorWitness
  ...
  | ReplayWitness
      { key :: BehaviorKey
      , historyPrefix :: [SurgeEvent]
      , observedChunk :: [SurgeEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  [ -- SurgeIdle x NoteSurgeThreshold: live transition (spec line 43)
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-86bc21fdc4a8ca5d"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = SurgeIdle
      , commandName = "NoteSurgeThreshold"
      , expectedEdge = (Just (K.EdgeRef SurgeIdle 0))
      , target = Just SurgeIdle
      , eventKinds = ["SurgeThresholdNoted"]
      , line = 43
      }
  , -- SurgeIdle x MarkSurgeTimerFired: live transition (spec line 44)
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-86bd3f785d9c7719"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = SurgeIdle
      , commandName = "MarkSurgeTimerFired"
      , expectedEdge = (Just (K.EdgeRef SurgeIdle 1))
      , target = Just SurgeFired
      , eventKinds = ["SurgeTimerMarked"]
      , line = 44
      }
  , -- SurgeFired x NoteSurgeThreshold: required rejection (spec line 37)
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-d517bd7075620508"
  ...
  , -- SurgeFired x MarkSurgeTimerFired: required rejection (spec line 37)
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-fe2793e47f0b6989"
      , kind = RequiredRejection
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = SurgeFired
