...
module SkelProcess.Generated.MyService.Surge.BehaviorContract
  ( BehaviorKey (..)
  , ObligationKind (..)
  , EvidenceLevel (..)
  , GuardCoverage (..)
...
data BehaviorWitness
  ...
  | LiveWitness
      ...
      , witnessHistory :: [SurgeEvent]
      , witnessCommand :: SurgeCommand
      , witnessExpected :: LiveExpectation
      }
  | ReplayWitness
      { witnessKey :: BehaviorKey
      , witnessHistoryPrefix :: [SurgeEvent]
      , witnessObservedChunk :: [SurgeEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  [ -- SurgeIdle x NoteSurgeThreshold: live transition (spec line 43)
    BehaviorRequirement
      { requirementKey = BehaviorKey "behavior-v1-86bc21fdc4a8ca5d"
      , requirementKind = LiveTransition
      , requirementEvidence = GeneratedAuthoritative
      , requirementGuardCoverage = GuardTotal
      , requirementSource = SurgeIdle
      , requirementCommandName = "NoteSurgeThreshold"
      , requirementExpectedEdge = (Just (K.EdgeRef SurgeIdle 0))
      , requirementTarget = Just SurgeIdle
      , requirementEventKinds = ["SurgeThresholdNoted"]
      , requirementLine = 43
      }
  , -- SurgeIdle x MarkSurgeTimerFired: live transition (spec line 44)
    BehaviorRequirement
      { requirementKey = BehaviorKey "behavior-v1-86bd3f785d9c7719"
      , requirementKind = LiveTransition
      , requirementEvidence = GeneratedAuthoritative
      , requirementGuardCoverage = GuardTotal
      , requirementSource = SurgeIdle
      , requirementCommandName = "MarkSurgeTimerFired"
      , requirementExpectedEdge = (Just (K.EdgeRef SurgeIdle 1))
      , requirementTarget = Just SurgeFired
      , requirementEventKinds = ["SurgeTimerMarked"]
      , requirementLine = 44
      }
  , -- SurgeFired x NoteSurgeThreshold: required rejection (spec line 37)
    BehaviorRequirement
      { requirementKey = BehaviorKey "behavior-v1-d517bd7075620508"
      , requirementKind = RequiredRejection
      , requirementEvidence = GeneratedAuthoritative
      , requirementGuardCoverage = GuardNotApplicable
      , requirementSource = SurgeFired
      , requirementCommandName = "NoteSurgeThreshold"
  ...
  , -- SurgeFired x MarkSurgeTimerFired: required rejection (spec line 37)
    BehaviorRequirement
      { requirementKey = BehaviorKey "behavior-v1-fe2793e47f0b6989"
      , requirementKind = RequiredRejection
      , requirementEvidence = GeneratedAuthoritative
      , requirementGuardCoverage = GuardNotApplicable
      , requirementSource = SurgeFired
...
runLive :: BehaviorRequirement -> [SurgeEvent] -> SurgeCommand -> LiveExpectation -> Either BehaviorFailure ()
...
runRejection :: BehaviorRequirement -> (SurgeVertex, K.RegFile SurgeRegs) -> SurgeCommand -> LiveExpectation -> Either BehaviorFailure ()
runRejection requirement seed command expectation = case expectation of
  ...
  Rejects expectedClass -> case K.stepDetailedEither surgeTransducer seed command of
...
runAcceptance :: BehaviorRequirement -> (SurgeVertex, K.RegFile SurgeRegs) -> SurgeCommand -> LiveExpectation -> Either BehaviorFailure ()
runAcceptance requirement seed command expectation = case expectation of
  ...
  NoOp -> case K.stepDetailedEither surgeTransducer seed command of
  ...
  Emits expectedEvents -> case K.stepDetailedEither surgeTransducer seed command of
    Left stepFailure -> failure requirement "unexpected-rejection" (tshow stepFailure)
    Right success -> do
      checkAcceptedEnvelope requirement success
      ...
      replayed <- case K.applyEventsDetailedEither surgeTransducer seed decoded of
...
checkAcceptedEnvelope :: BehaviorRequirement -> K.StepSuccess SurgeRegs SurgeVertex SurgeEvent -> Either BehaviorFailure ()
...
runReplay :: BehaviorRequirement -> [SurgeEvent] -> [SurgeEvent] -> Either BehaviorFailure ()
runReplay requirement prefix chunk = case requirementKind requirement of
  ReplayTransition -> do
    ...
    replayed <- case K.applyEventsDetailedEither surgeTransducer (K.replaySuccessState settled, K.replaySuccessRegs settled) decoded of
...
checkSingleAttribution :: BehaviorRequirement -> K.EdgeMode -> Int -> [K.ReplayAttribution SurgeVertex] -> Either BehaviorFailure ()
...
settleHistory :: BehaviorRequirement -> Text -> [SurgeEvent] -> Either BehaviorFailure (K.ReplaySuccess SurgeRegs SurgeVertex)
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither surgeTransducer (SurgeIdle, initialSurgeRegs) decoded of
    Left replayFailure -> failure requirement (label <> "-replay-failed") (tshow replayFailure)
    Right replaySuccess -> Right replaySuccess

decodeEvents :: [SurgeEvent] -> Either Text [SurgeEvent]
decodeEvents = traverse (\event -> parseSurgeEvent (Codec.eventType surgeCodec event) (encodeSurgeEvent event))

commandKind :: SurgeCommand -> Text
commandKind command = case command of
  NoteSurgeThreshold _ -> "NoteSurgeThreshold"
  MarkSurgeTimerFired _ -> "MarkSurgeTimerFired"

eventKind :: SurgeEvent -> Text
eventKind event = case Codec.eventType surgeCodec event of Codec.EventType tag -> tag

regsEqual :: K.RegFile SurgeRegs -> K.RegFile SurgeRegs -> Bool
regsEqual _ _ = True
