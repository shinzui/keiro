...
behaviorRequirements =
  [ -- HospitalOperational x ActivateSurge: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-69ac3daa8569d350"
      , kind = LiveTransition
      ...
      , target = Just HospitalOperational
...
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither hospitalTransducer (HospitalOperational, initialHospitalRegs) decoded of
