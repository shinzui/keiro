...
behaviorRequirements =
  [ -- ReservationEligible x Cancel: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-0b1588f69a27bc77"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = ReservationEligible
      , commandName = "Cancel"
      , expectedEdge = (Just (K.EdgeRef ReservationEligible 0))
      , target = Just ReservationCancelledState
      , eventKinds = ["Cancelled"]
      }
  , -- ReservationCancelledState x Cancel: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-316c9e96fc1a94b1"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = ReservationCancelledState
      , commandName = "Cancel"
      , expectedEdge = (Just (K.EdgeRef ReservationCancelledState 0))
      , target = Just ReservationCancelledState
      , eventKinds = []
      }
  , -- ReservationCancelledState x Cancel: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-fd391b55fbbfa640"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardTotal
      , source = ReservationCancelledState
      , commandName = "Cancel"
      , expectedEdge = (Just (K.EdgeRef ReservationCancelledState 1))
      , target = Just ReservationCancelledState
      , eventKinds = []
      }
  ]
