...
import Generated.StructuralNominalLeaves.TemplateCatalog.Codec (encodeTemplateCatalogEvent, parseTemplateCatalogEvent, templateCatalogCodec)
...
import Generated.StructuralNominalLeaves.BehaviorSourceMap qualified as BehaviorSourceMap
...
data BehaviorRequirement = BehaviorRequirement
  ...
  , source :: !TemplateCatalogVertex
  , commandName :: !Text
  , expectedEdge :: !(Maybe (K.EdgeRef TemplateCatalogVertex))
  , target :: !(Maybe TemplateCatalogVertex)
  , eventKinds :: ![Text]
  }
...
data LiveExpectation
  = Emits (NonEmpty TemplateCatalogEvent)
...
data BehaviorWitness
  ...
  | LiveWitness
      { key :: BehaviorKey
      , history :: [TemplateCatalogEvent]
      , command :: TemplateCatalogCommand
      , expected :: LiveExpectation
      }
  | ReplayWitness
      { key :: BehaviorKey
      , historyPrefix :: [TemplateCatalogEvent]
      , observedChunk :: [TemplateCatalogEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  ...
  , -- TemplateCatalogRecorded x RecordTemplate: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-6cb89110a4e5d4bc"
      , kind = RequiredRejection
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = TemplateCatalogRecorded
...
renderBehaviorConformanceText report = T.unlines
  [ "behavior conformance: TemplateCatalog"
...
runLive :: BehaviorRequirement -> [TemplateCatalogEvent] -> TemplateCatalogCommand -> LiveExpectation -> Either BehaviorFailure ()
...
runRejection :: BehaviorRequirement -> (TemplateCatalogVertex, K.RegFile TemplateCatalogRegs) -> TemplateCatalogCommand -> LiveExpectation -> Either BehaviorFailure ()
runRejection requirement seed command expectation = case expectation of
  ...
  Rejects expectedClass -> case K.stepDetailedEither templateCatalogTransducer seed command of
...
runAcceptance :: BehaviorRequirement -> (TemplateCatalogVertex, K.RegFile TemplateCatalogRegs) -> TemplateCatalogCommand -> LiveExpectation -> Either BehaviorFailure ()
runAcceptance requirement seed command expectation = case expectation of
  ...
  NoOp -> case K.stepDetailedEither templateCatalogTransducer seed command of
  ...
  Emits expectedEvents -> case K.stepDetailedEither templateCatalogTransducer seed command of
    Left stepFailure -> failure requirement "unexpected-rejection" (tshow stepFailure)
    Right success -> do
      checkAcceptedEnvelope requirement success
      ...
      replayed <- case K.applyEventsDetailedEither templateCatalogTransducer seed decoded of
...
checkAcceptedEnvelope :: BehaviorRequirement -> K.StepSuccess TemplateCatalogRegs TemplateCatalogVertex TemplateCatalogEvent -> Either BehaviorFailure ()
...
runReplay :: BehaviorRequirement -> [TemplateCatalogEvent] -> [TemplateCatalogEvent] -> Either BehaviorFailure ()
runReplay requirement prefix chunk = case requirement.kind of
  ReplayTransition -> do
    ...
    replayed <- case K.applyEventsDetailedEither templateCatalogTransducer (K.replaySuccessState settled, K.replaySuccessRegs settled) decoded of
...
checkSingleAttribution :: BehaviorRequirement -> K.EdgeMode -> Int -> [K.ReplayAttribution TemplateCatalogVertex] -> Either BehaviorFailure ()
...
settleHistory :: BehaviorRequirement -> Text -> [TemplateCatalogEvent] -> Either BehaviorFailure (K.ReplaySuccess TemplateCatalogRegs TemplateCatalogVertex)
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither templateCatalogTransducer (TemplateCatalogEmpty, initialTemplateCatalogRegs) decoded of
    Left replayFailure -> failure requirement (label <> "-replay-failed") (tshow replayFailure)
    Right replaySuccess -> Right replaySuccess

decodeEvents :: [TemplateCatalogEvent] -> Either Text [TemplateCatalogEvent]
decodeEvents = traverse (\event -> parseTemplateCatalogEvent (Codec.eventType templateCatalogCodec event) (encodeTemplateCatalogEvent event))
