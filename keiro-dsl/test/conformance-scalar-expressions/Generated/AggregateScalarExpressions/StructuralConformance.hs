-- @generated by keiro-dsl 0.16.0.0 (language keiro-dsl 4) from context aggregate-scalar-expressions structural conformance; do not edit.
...
structuralProjectionAssertions =
  [ ("projection witness agreement: scalar-expressions.Limits.v1/ceiling", all (\(_, owner) -> fieldWitnessAgrees StructuralProjections.limitsCeilingWitness (\referenceOwner -> StructuralProjections.limitsCeilingGet referenceOwner) owner) (NonEmpty.toList (fixtureCases Bindings.limitsCases)))
  , ("projection witness agreement: scalar-expressions.Limits.v1/minimum", all (\(_, owner) -> fieldWitnessAgrees StructuralProjections.limitsMinimumWitness (\referenceOwner -> StructuralProjections.limitsMinimumGet referenceOwner) owner) (NonEmpty.toList (fixtureCases Bindings.limitsCases)))
  ]
