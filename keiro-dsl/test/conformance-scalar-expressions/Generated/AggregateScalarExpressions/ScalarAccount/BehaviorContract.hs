...
data BehaviorRequirement = BehaviorRequirement
  ...
  , source :: !ScalarAccountVertex
  , commandName :: !Text
  , expectedEdge :: !(Maybe (K.EdgeRef ScalarAccountVertex))
  , target :: !(Maybe ScalarAccountVertex)
  , eventKinds :: ![Text]
  }
...
data LiveExpectation
  = Emits (NonEmpty ScalarAccountEvent)
...
data BehaviorWitness
  ...
  | LiveWitness
      { key :: BehaviorKey
      , history :: [ScalarAccountEvent]
      , command :: ScalarAccountCommand
      , expected :: LiveExpectation
      }
  | ReplayWitness
      { key :: BehaviorKey
      , historyPrefix :: [ScalarAccountEvent]
      , observedChunk :: [ScalarAccountEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  ...
  , -- ScalarAccountReviewed x Adjust: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-5c58e0aafeb5f400"
      , kind = RequiredRejection
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = ScalarAccountReviewed
      , commandName = "Adjust"
      , expectedEdge = Nothing
      , target = Nothing
      , eventKinds = []
  ...
  , -- ScalarAccountClosed x Adjust: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-a19a8e94d935493f"
      , kind = RequiredRejection
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = ScalarAccountClosed
      , commandName = "Adjust"
      , expectedEdge = Nothing
      , target = Nothing
      , eventKinds = []
  ...
  , -- ScalarAccountReviewed x Close: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-b7db52f28632d8da"
      , kind = LiveTransition
      , evidence = HoleWitnessed
      , guardCoverage = GuardTotal
      , source = ScalarAccountReviewed
      , commandName = "Close"
      , expectedEdge = (Just (K.EdgeRef ScalarAccountReviewed 0))
      , target = Just ScalarAccountClosed
      , eventKinds = ["ClosedEvent"]
      }
  , -- ScalarAccountClosed x Close: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-dbf483c868dd6d34"
      , kind = RequiredRejection
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = ScalarAccountClosed
  ...
  , -- ScalarAccountOpen x Adjust: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-e9c863d5daf20011"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardUnknown
      , source = ScalarAccountOpen
      , commandName = "Adjust"
      , expectedEdge = (Just (K.EdgeRef ScalarAccountOpen 0))
      , target = Just ScalarAccountReviewed
      , eventKinds = ["Adjusted"]
      }
  ]
...
renderBehaviorConformanceText report = T.unlines
  [ "behavior conformance: ScalarAccount"
...
runLive :: BehaviorRequirement -> [ScalarAccountEvent] -> ScalarAccountCommand -> LiveExpectation -> Either BehaviorFailure ()
...
runRejection :: BehaviorRequirement -> (ScalarAccountVertex, K.RegFile ScalarAccountRegs) -> ScalarAccountCommand -> LiveExpectation -> Either BehaviorFailure ()
runRejection requirement seed command expectation = case expectation of
  ...
  Rejects expectedClass -> case K.stepDetailedEither scalarAccountTransducer seed command of
...
runAcceptance :: BehaviorRequirement -> (ScalarAccountVertex, K.RegFile ScalarAccountRegs) -> ScalarAccountCommand -> LiveExpectation -> Either BehaviorFailure ()
runAcceptance requirement seed command expectation = case expectation of
  ...
  NoOp -> case K.stepDetailedEither scalarAccountTransducer seed command of
  ...
  Emits expectedEvents -> case K.stepDetailedEither scalarAccountTransducer seed command of
    Left stepFailure -> failure requirement "unexpected-rejection" (tshow stepFailure)
    Right success -> do
      checkAcceptedEnvelope requirement success
      ...
      replayed <- case K.applyEventsDetailedEither scalarAccountTransducer seed decoded of
...
checkAcceptedEnvelope :: BehaviorRequirement -> K.StepSuccess ScalarAccountRegs ScalarAccountVertex ScalarAccountEvent -> Either BehaviorFailure ()
...
runReplay :: BehaviorRequirement -> [ScalarAccountEvent] -> [ScalarAccountEvent] -> Either BehaviorFailure ()
runReplay requirement prefix chunk = case requirement.kind of
  ReplayTransition -> do
    ...
    replayed <- case K.applyEventsDetailedEither scalarAccountTransducer (K.replaySuccessState settled, K.replaySuccessRegs settled) decoded of
...
checkSingleAttribution :: BehaviorRequirement -> K.EdgeMode -> Int -> [K.ReplayAttribution ScalarAccountVertex] -> Either BehaviorFailure ()
...
settleHistory :: BehaviorRequirement -> Text -> [ScalarAccountEvent] -> Either BehaviorFailure (K.ReplaySuccess ScalarAccountRegs ScalarAccountVertex)
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither scalarAccountTransducer (ScalarAccountOpen, initialScalarAccountRegs) decoded of
...
decodeEvents = traverse (\event -> parseScalarAccountEvent (Codec.eventType scalarAccountCodec event) (encodeScalarAccountEvent event))

commandKind :: ScalarAccountCommand -> Text
commandKind command = case command of
  Adjust _ -> "Adjust"
  Close _ -> "Close"

eventKind :: ScalarAccountEvent -> Text
eventKind event = case Codec.eventType scalarAccountCodec event of Codec.EventType tag -> tag

regsEqual :: K.RegFile ScalarAccountRegs -> K.RegFile ScalarAccountRegs -> Bool
regsEqual left right = (left K.! #balance) == (right K.! #balance) && (left K.! #reserved) == (right K.! #reserved) && (left K.! #capacity) == (right K.! #capacity) && (left K.! #machine) == (right K.! #machine) && (left K.! #label) == (right K.! #label) && (left K.! #active) == (right K.! #active) && (left K.! #mode) == (right K.! #mode) && (left K.! #requestId) == (right K.! #requestId) && (left K.! #openedAt) == (right K.! #openedAt) && (left K.! #limits) == (right K.! #limits)
