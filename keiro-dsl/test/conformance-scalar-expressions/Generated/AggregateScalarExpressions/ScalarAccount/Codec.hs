...
import Generated.AggregateScalarExpressions.Nominals (AccountMode (..), accountModeText, requestIdText)
...
encodeLimitsShape shape =
  object
      [ "minimum" .= toJSON (shape.minimum)
      , "ceiling" .= toJSON (shape.ceiling)
      ]
  ...
  where
    go o = do
      case tag of
        ...
        _ -> fail ("unknown event type " <> show tag <> "; expected one of: " <> renderExpectedEventTypes scalarAccountEventTypes)
