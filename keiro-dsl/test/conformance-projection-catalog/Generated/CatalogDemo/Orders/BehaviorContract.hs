...
data BehaviorRequirement = BehaviorRequirement
  ...
  , requirementSource :: !OrdersVertex
  , requirementCommandName :: !Text
  , requirementExpectedEdge :: !(Maybe (K.EdgeRef OrdersVertex))
  , requirementTarget :: !(Maybe OrdersVertex)
...
data BehaviorWitness
  ...
  | LiveWitness
      ...
      , witnessHistory :: [OrdersEvent]
      , witnessCommand :: OrdersCommand
      , witnessExpected :: LiveExpectation
      }
  | ReplayWitness
      { witnessKey :: BehaviorKey
      , witnessHistoryPrefix :: [OrdersEvent]
      , witnessObservedChunk :: [OrdersEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  [ -- OrdersEmpty x RecordOrder: live transition
    BehaviorRequirement
      { requirementKey = BehaviorKey "behavior-v1-8861ebe94ff5f68a"
      , requirementKind = LiveTransition
      , requirementEvidence = GeneratedAuthoritative
      , requirementGuardCoverage = GuardTotal
      , requirementSource = OrdersEmpty
      , requirementCommandName = "RecordOrder"
      , requirementExpectedEdge = (Just (K.EdgeRef OrdersEmpty 0))
      , requirementTarget = Just OrdersRecorded
      , requirementEventKinds = ["OrderRecorded"]
      }
  , -- OrdersRecorded x RecordOrder: required rejection
    BehaviorRequirement
      { requirementKey = BehaviorKey "behavior-v1-badd7ebab3a3a844"
      , requirementKind = RequiredRejection
      , requirementEvidence = GeneratedAuthoritative
      , requirementGuardCoverage = GuardNotApplicable
      , requirementSource = OrdersRecorded
      , requirementCommandName = "RecordOrder"
...
runLive :: BehaviorRequirement -> [OrdersEvent] -> OrdersCommand -> LiveExpectation -> Either BehaviorFailure ()
...
runRejection :: BehaviorRequirement -> (OrdersVertex, K.RegFile OrdersRegs) -> OrdersCommand -> LiveExpectation -> Either BehaviorFailure ()
runRejection requirement seed command expectation = case expectation of
  ...
  Rejects expectedClass -> case K.stepDetailedEither ordersTransducer seed command of
...
runAcceptance :: BehaviorRequirement -> (OrdersVertex, K.RegFile OrdersRegs) -> OrdersCommand -> LiveExpectation -> Either BehaviorFailure ()
runAcceptance requirement seed command expectation = case expectation of
  ...
  NoOp -> case K.stepDetailedEither ordersTransducer seed command of
  ...
  Emits expectedEvents -> case K.stepDetailedEither ordersTransducer seed command of
    Left stepFailure -> failure requirement "unexpected-rejection" (tshow stepFailure)
    Right success -> do
      checkAcceptedEnvelope requirement success
      ...
      replayed <- case K.applyEventsDetailedEither ordersTransducer seed decoded of
...
checkAcceptedEnvelope :: BehaviorRequirement -> K.StepSuccess OrdersRegs OrdersVertex OrdersEvent -> Either BehaviorFailure ()
...
runReplay :: BehaviorRequirement -> [OrdersEvent] -> [OrdersEvent] -> Either BehaviorFailure ()
runReplay requirement prefix chunk = case requirementKind requirement of
  ReplayTransition -> do
    ...
    replayed <- case K.applyEventsDetailedEither ordersTransducer (K.replaySuccessState settled, K.replaySuccessRegs settled) decoded of
...
checkSingleAttribution :: BehaviorRequirement -> K.EdgeMode -> Int -> [K.ReplayAttribution OrdersVertex] -> Either BehaviorFailure ()
...
settleHistory :: BehaviorRequirement -> Text -> [OrdersEvent] -> Either BehaviorFailure (K.ReplaySuccess OrdersRegs OrdersVertex)
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither ordersTransducer (OrdersEmpty, initialOrdersRegs) decoded of
    Left replayFailure -> failure requirement (label <> "-replay-failed") (tshow replayFailure)
    Right replaySuccess -> Right replaySuccess

decodeEvents :: [OrdersEvent] -> Either Text [OrdersEvent]
decodeEvents = traverse (\event -> parseOrdersEvent (Codec.eventType ordersCodec event) (encodeOrdersEvent event))

commandKind :: OrdersCommand -> Text
commandKind command = case command of
  RecordOrder _ -> "RecordOrder"

eventKind :: OrdersEvent -> Text
