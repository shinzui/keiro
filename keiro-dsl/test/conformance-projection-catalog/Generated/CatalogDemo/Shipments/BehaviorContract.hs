...
import Generated.CatalogDemo.BehaviorSourceMap qualified as BehaviorSourceMap
...
data BehaviorRequirement = BehaviorRequirement
  ...
  , requirementSource :: !ShipmentsVertex
  , requirementCommandName :: !Text
  , requirementExpectedEdge :: !(Maybe (K.EdgeRef ShipmentsVertex))
  , requirementTarget :: !(Maybe ShipmentsVertex)
...
data BehaviorWitness
  ...
  | LiveWitness
      ...
      , witnessHistory :: [ShipmentsEvent]
      , witnessCommand :: ShipmentsCommand
      , witnessExpected :: LiveExpectation
      }
  | ReplayWitness
      { witnessKey :: BehaviorKey
      , witnessHistoryPrefix :: [ShipmentsEvent]
      , witnessObservedChunk :: [ShipmentsEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  [ -- ShipmentsRecorded x RecordShipment: required rejection
    BehaviorRequirement
      { requirementKey = BehaviorKey "behavior-v1-89eeb23ded471fe8"
      , requirementKind = RequiredRejection
      , requirementEvidence = GeneratedAuthoritative
      , requirementGuardCoverage = GuardNotApplicable
      , requirementSource = ShipmentsRecorded
      , requirementCommandName = "RecordShipment"
      , requirementExpectedEdge = Nothing
      , requirementTarget = Nothing
      , requirementEventKinds = []
      }
  , -- ShipmentsEmpty x RecordShipment: live transition
    BehaviorRequirement
      { requirementKey = BehaviorKey "behavior-v1-b0a7e39ecba10454"
      , requirementKind = LiveTransition
      , requirementEvidence = GeneratedAuthoritative
      , requirementGuardCoverage = GuardTotal
      , requirementSource = ShipmentsEmpty
      , requirementCommandName = "RecordShipment"
      , requirementExpectedEdge = (Just (K.EdgeRef ShipmentsEmpty 0))
      , requirementTarget = Just ShipmentsRecorded
      , requirementEventKinds = ["ShipmentRecorded"]
      }
  ]
...
renderBehaviorConformanceText report = T.unlines
  [ "behavior conformance: Shipments"
...
runLive :: BehaviorRequirement -> [ShipmentsEvent] -> ShipmentsCommand -> LiveExpectation -> Either BehaviorFailure ()
...
runRejection :: BehaviorRequirement -> (ShipmentsVertex, K.RegFile ShipmentsRegs) -> ShipmentsCommand -> LiveExpectation -> Either BehaviorFailure ()
runRejection requirement seed command expectation = case expectation of
  ...
  Rejects expectedClass -> case K.stepDetailedEither shipmentsTransducer seed command of
...
runAcceptance :: BehaviorRequirement -> (ShipmentsVertex, K.RegFile ShipmentsRegs) -> ShipmentsCommand -> LiveExpectation -> Either BehaviorFailure ()
runAcceptance requirement seed command expectation = case expectation of
  ...
  NoOp -> case K.stepDetailedEither shipmentsTransducer seed command of
  ...
  Emits expectedEvents -> case K.stepDetailedEither shipmentsTransducer seed command of
    Left stepFailure -> failure requirement "unexpected-rejection" (tshow stepFailure)
    Right success -> do
      checkAcceptedEnvelope requirement success
      ...
      replayed <- case K.applyEventsDetailedEither shipmentsTransducer seed decoded of
...
checkAcceptedEnvelope :: BehaviorRequirement -> K.StepSuccess ShipmentsRegs ShipmentsVertex ShipmentsEvent -> Either BehaviorFailure ()
...
runReplay :: BehaviorRequirement -> [ShipmentsEvent] -> [ShipmentsEvent] -> Either BehaviorFailure ()
runReplay requirement prefix chunk = case requirementKind requirement of
  ReplayTransition -> do
    ...
    replayed <- case K.applyEventsDetailedEither shipmentsTransducer (K.replaySuccessState settled, K.replaySuccessRegs settled) decoded of
...
checkSingleAttribution :: BehaviorRequirement -> K.EdgeMode -> Int -> [K.ReplayAttribution ShipmentsVertex] -> Either BehaviorFailure ()
...
settleHistory :: BehaviorRequirement -> Text -> [ShipmentsEvent] -> Either BehaviorFailure (K.ReplaySuccess ShipmentsRegs ShipmentsVertex)
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither shipmentsTransducer (ShipmentsEmpty, initialShipmentsRegs) decoded of
    Left replayFailure -> failure requirement (label <> "-replay-failed") (tshow replayFailure)
    Right replaySuccess -> Right replaySuccess

decodeEvents :: [ShipmentsEvent] -> Either Text [ShipmentsEvent]
decodeEvents = traverse (\event -> parseShipmentsEvent (Codec.eventType shipmentsCodec event) (encodeShipmentsEvent event))

commandKind :: ShipmentsCommand -> Text
commandKind command = case command of
  RecordShipment _ -> "RecordShipment"

eventKind :: ShipmentsEvent -> Text
eventKind event = case Codec.eventType shipmentsCodec event of Codec.EventType tag -> tag

regsEqual :: K.RegFile ShipmentsRegs -> K.RegFile ShipmentsRegs -> Bool
regsEqual left right = (left K.! #accepted) == (right K.! #accepted)
