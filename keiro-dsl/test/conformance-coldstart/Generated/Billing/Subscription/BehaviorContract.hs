...
-- @generated by keiro-dsl 0.16.0.0 (language keiro-dsl 4) from aggregate Subscription; do not edit.
...
data BehaviorRequirement = BehaviorRequirement
  ...
  , source :: !SubscriptionVertex
  , commandName :: !Text
  , expectedEdge :: !(Maybe (K.EdgeRef SubscriptionVertex))
  , target :: !(Maybe SubscriptionVertex)
  , eventKinds :: ![Text]
  }
...
data LiveExpectation
  = Emits (NonEmpty SubscriptionEvent)
...
data BehaviorWitness
  ...
  | ReplayWitness
      { key :: BehaviorKey
      , historyPrefix :: [SubscriptionEvent]
      , observedChunk :: [SubscriptionEvent]
      }
  deriving stock (Eq, Show)
...
behaviorRequirements =
  ...
  , -- SubscriptionInactive x ActivateSubscription: live transition
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-9399755ba2b617d6"
      , kind = LiveTransition
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardUnknown
      , source = SubscriptionInactive
      , commandName = "ActivateSubscription"
      , expectedEdge = (Just (K.EdgeRef SubscriptionInactive 0))
      , target = Just SubscriptionActive
      , eventKinds = ["SubscriptionActivated"]
      }
  , -- SubscriptionInactive x CancelSubscription: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-93f6f3f8cb36420d"
      , kind = RequiredRejection
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = SubscriptionInactive
  ...
  , -- SubscriptionClosed x CancelSubscription: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-99ac0c974e512f76"
      , kind = RequiredRejection
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = SubscriptionClosed
      , commandName = "CancelSubscription"
      , expectedEdge = Nothing
      , target = Nothing
      , eventKinds = []
  ...
  , -- SubscriptionClosed x ActivateSubscription: required rejection
    BehaviorRequirement
      { key = BehaviorKey "behavior-v1-9b81e070774b3ae1"
      , kind = RequiredRejection
      , evidence = GeneratedAuthoritative
      , guardCoverage = GuardNotApplicable
      , source = SubscriptionClosed
  ...
  , -- SubscriptionActive x CancelSubscription: live transition
    BehaviorRequirement
      ...
      , source = SubscriptionActive
      , commandName = "CancelSubscription"
      , expectedEdge = (Just (K.EdgeRef SubscriptionActive 0))
      , target = Just SubscriptionClosed
      , eventKinds = ["SubscriptionCancelled"]
      }
  ]
...
renderBehaviorConformanceText report = T.unlines
  [ "behavior conformance: Subscription"
...
runLive :: BehaviorRequirement -> [SubscriptionEvent] -> SubscriptionCommand -> LiveExpectation -> Either BehaviorFailure ()
...
runRejection :: BehaviorRequirement -> (SubscriptionVertex, K.RegFile SubscriptionRegs) -> SubscriptionCommand -> LiveExpectation -> Either BehaviorFailure ()
runRejection requirement seed command expectation = case expectation of
  ...
  Rejects expectedClass -> case K.stepDetailedEither subscriptionTransducer seed command of
...
runAcceptance :: BehaviorRequirement -> (SubscriptionVertex, K.RegFile SubscriptionRegs) -> SubscriptionCommand -> LiveExpectation -> Either BehaviorFailure ()
runAcceptance requirement seed command expectation = case expectation of
  ...
  NoOp -> case K.stepDetailedEither subscriptionTransducer seed command of
  ...
  Emits expectedEvents -> case K.stepDetailedEither subscriptionTransducer seed command of
    Left stepFailure -> failure requirement "unexpected-rejection" (tshow stepFailure)
    Right success -> do
      checkAcceptedEnvelope requirement success
      ...
      replayed <- case K.applyEventsDetailedEither subscriptionTransducer seed decoded of
...
checkAcceptedEnvelope :: BehaviorRequirement -> K.StepSuccess SubscriptionRegs SubscriptionVertex SubscriptionEvent -> Either BehaviorFailure ()
...
runReplay :: BehaviorRequirement -> [SubscriptionEvent] -> [SubscriptionEvent] -> Either BehaviorFailure ()
runReplay requirement prefix chunk = case requirement.kind of
  ReplayTransition -> do
    ...
    replayed <- case K.applyEventsDetailedEither subscriptionTransducer (K.replaySuccessState settled, K.replaySuccessRegs settled) decoded of
...
checkSingleAttribution :: BehaviorRequirement -> K.EdgeMode -> Int -> [K.ReplayAttribution SubscriptionVertex] -> Either BehaviorFailure ()
...
settleHistory :: BehaviorRequirement -> Text -> [SubscriptionEvent] -> Either BehaviorFailure (K.ReplaySuccess SubscriptionRegs SubscriptionVertex)
settleHistory requirement label history = do
  ...
  case K.applyEventsDetailedEither subscriptionTransducer (SubscriptionInactive, initialSubscriptionRegs) decoded of
    Left replayFailure -> failure requirement (label <> "-replay-failed") (tshow replayFailure)
    Right replaySuccess -> Right replaySuccess

decodeEvents :: [SubscriptionEvent] -> Either Text [SubscriptionEvent]
decodeEvents = traverse (\event -> parseSubscriptionEvent (Codec.eventType subscriptionCodec event) (encodeSubscriptionEvent event))

commandKind :: SubscriptionCommand -> Text
commandKind command = case command of
  ActivateSubscription _ -> "ActivateSubscription"
  CancelSubscription _ -> "CancelSubscription"

eventKind :: SubscriptionEvent -> Text
eventKind event = case Codec.eventType subscriptionCodec event of Codec.EventType tag -> tag

regsEqual :: K.RegFile SubscriptionRegs -> K.RegFile SubscriptionRegs -> Bool
regsEqual left right = (left K.! #plan) == (right K.! #plan)
